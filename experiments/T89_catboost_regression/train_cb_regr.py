"""T89: CatBoost regression on Δmid_norm — Optiver 2023 1st-place style.

Target: y_regr = (mp_t60 - mp_t) / (mp_t + 1)  (matches PnL formula denom)
Features: schemeP (T68) cache, drop set = T59 fail (10) + Stage 5 fail (1) → 359 dims.
Split: V4 walk-forward — train date 0-75, val date 76-79, test 96-119 (5 sym).
Training: CatBoost RMSE, GPU (task_type='GPU'), 5 seed × 1 model.
aug_a [0.80, 1.20] applied; sample weights = class-balanced on the 3-class label.

Saves model_T89_seed{S}.cbm and pred_T89_seed{S}.parquet (with predicted Δmid_norm).
"""
...
# Per-seed configs: vary depth and l2_leaf_reg for ensemble diversity
SEED_CONFIGS = {
...
def main():
    ...
    H = args.horizon
    seed = args.seed
    print(f"=== T89 CatBoost regression Δmid seed={seed} h={H} obj=RMSE ===", flush=True)
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = (f"T89-cb-regr-dmid-seed{seed}-RMSE-"
            ...
            wandb.init(
                ...
                config={
                    "model": "catboost-gpu",
                    "task": "T89_catboost_regression_dmid",
                    "objective": "RMSE",
                    "split_info": info,
                    "n_features": feat_dim,
                    "horizon": H,
                ...
                tags=["T89", "catboost", "regression", "EV-gate", f"h{H}", "RMSE"],
            )
    ...
    model_path = os.path.join(HERE, f"model_T89_seed{seed}.cbm")
    ...
    pred_path = os.path.join(HERE, f"pred_T89_seed{seed}.parquet")
    ...
    summary = {
        "task": f"T89 CatBoost regression Δmid seed={seed}",
        "seed": seed,
        "horizon": H,
        "objective": "RMSE",
        "split_info": info,
    ...
    out_path = os.path.join(HERE, f"summary_T89_seed{seed}.json")
