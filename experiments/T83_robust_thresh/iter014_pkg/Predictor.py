"""Predictor for iter_013: T75 regression-on-Δmid + EV-gate.

Replaces iter_012's 3-class CE + DE 4D probability thresholds with:
  * 5-seed LightGBM `objective=regression_l2` predicting Δmid_norm =
    (mp_t60 - mp_t) / (mp_t + 1)
  * Asymmetric EV gate: take long when pred_avg > thr_up, short when
    pred_avg < -thr_dn, else flat. Thresholds live in thresholds.json.

Feature pipeline is **identical** to iter_012 (same fast_features_batch.py,
same 359 dims after dropping 11 KS-fail extras). Only the boosters and
...
Compliance with platform contract (CRITICAL_CONSTRAINTS.md):
  ...
  - sym-agnostic: features only use 100-tick LOB window, no sym-specific
    normalization, and the EV-gate threshold is a single global pair
...
# Same blocked raw column order as the T68 cache used for training.
...
class Predictor:
    ...
    def _ev_gate_predict(pred_dmid: np.ndarray, hcfg: Dict) -> np.ndarray:
        ...
        Take long (action=2) when pred_dmid > thr_up,
        take short (action=0) when pred_dmid < -thr_dn,
        else flat (action=1).

        thr_up, thr_dn are positive numbers (the dn cutoff sits at -thr_dn).
        """
    ...
    def _compute_batch_features(self, batches: List[pd.DataFrame]) -> np.ndarray:
        """Same feature pipeline as iter_012 → (N, 359) float32."""
        N = len(batches)
    ...
    def _ensemble_predict(self, boosters: List[lgb.Booster], X: np.ndarray) -> np.ndarray:
        """Average regression predictions across ensemble seeds.

        For regression boosters, predict() returns (N,) Δmid_norm. We average
        across seeds to get the ensemble estimate.
        """
        if len(boosters) == 1:
    ...
    def predict(self, batches: List[pd.DataFrame]) -> List[List[int]]:
        ...
        for hcfg in self._horizons:
            ...
            if not boosters:
                continue
            pred_dmid = self._ensemble_predict(boosters, feats)
            preds = self._ev_gate_predict(pred_dmid, hcfg)
