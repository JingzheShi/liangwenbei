"""T37 DE 4D threshold search on 5-seed-averaged OOF h_60 predictions.

For a given step_tag (stepA or stepB), loads
    {step_tag}_pred_seed{S}_held{K}.parquet  for S in {42,1,7,13,100}, K in 0..4
averages probs across 5 seeds per fold, then runs:
  1) Coarse asymmetric grid sweep
  2) Differential evolution (4D continuous) over (T_up, T_dn, d_up, d_dn)
  3) Local neighborhood sanity check around DE optimum

Writes {step_tag}_thresh_results.json into HERE.
"""
...
def load_fold_avg(step_tag: str, k: int, seeds: List[int]) -> pd.DataFrame:
    base = pd.read_parquet(os.path.join(HERE, f"{step_tag}_pred_seed{seeds[0]}_held{k}.parquet"))
    p_sum = base[PROB_COLS].to_numpy(np.float64).copy()
    n = len(base)
    for s in seeds[1:]:
        df_s = pd.read_parquet(os.path.join(HERE, f"{step_tag}_pred_seed{s}_held{k}.parquet"))
        if len(df_s) != n:
            raise RuntimeError(f"row count mismatch step={step_tag} fold={k} seed={s}: {len(df_s)} vs {n}")
...
def per_fold_metrics(fa: FoldArrays, pred: np.ndarray) -> dict:
    m = _per_horizon_metrics(
        pred.astype(np.int64), fa.label,
        fa.mp_t.astype(np.float32), fa.mp_th.astype(np.float32),
        fee_rate=FEE,
    )
    return {
...
def main():
    ...
    ap.add_argument("--step-tag", required=True, choices=("stepA", "stepB"))
    ap.add_argument("--seeds", default=",".join(str(s) for s in SEEDS_DEFAULT))
    args = ap.parse_args()

    seeds = [int(s) for s in args.seeds.split(",")]
    print(f"=== T37 DE thresh search step={args.step_tag} seeds={seeds} ===", flush=True)

    t0 = time.time()
    folds = {k: load_fold_avg(args.step_tag, k, seeds) for k in range(N_FOLDS)}
    ...
    out_path = os.path.join(HERE, f"{args.step_tag}_thresh_results.json")
