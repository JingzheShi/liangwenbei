"""T71 DE 4D threshold search on 5-seed-averaged predictions for h_40.

Mirrors T70 de_eval_t70.py but loads pred_T71_h40_seed{S}.parquet.
"""
...
SYMS = (0, 1, 2, 3, 4)
ITER012_LOSO = 26.44  # iter_012 h_60 LOSO-equiv (the bar to beat)
ITER011_LOSO = 25.94


def load_avg_pred(seeds, horizon):
    base = pd.read_parquet(os.path.join(HERE, f"pred_T71_h{horizon}_seed{seeds[0]}.parquet"))
    p_sum = base[PROB_COLS].to_numpy(np.float64).copy()
    n = len(base)
    for s in seeds[1:]:
        df_s = pd.read_parquet(os.path.join(HERE, f"pred_T71_h{horizon}_seed{s}.parquet"))
        if len(df_s) != n:
...
def main():
    ...
    print(f"=== T71 DE 4D thresh h={H} seeds={seeds} ===", flush=True)

    t0 = time.time()
    df = load_avg_pred(seeds, H)
    ...
    out = {
        "task": "T71 DE 4D thresh (V4 walk-fwd + Stage 5 features, h40)",
        "experiment": "T71_h40_stage5",
        "horizon": H,
        "seeds": seeds,
        "n_test": len(df),
        "raw_argmax": {
        ...
        "compare_baselines": {
            "iter011_loso_h60": ITER011_LOSO,
            "iter012_loso_h60": ITER012_LOSO,
            "t71_h40_de_loso_sum_vs_iter012_h60": de_loso_sum - ITER012_LOSO,
            "t71_h40_de_loso_sum_vs_iter011_h60": de_loso_sum - ITER011_LOSO,
        },
    }
    out_path = os.path.join(HERE, f"de_results_t71_h{H}.json")
    ...
    print(f"vs iter_012 h60 +{ITER012_LOSO:.2f}: t71_h40_loso_diff={de_loso_sum - ITER012_LOSO:+.4f}", flush=True)
