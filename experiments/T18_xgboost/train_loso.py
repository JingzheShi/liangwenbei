...
Mirrors T11 (LightGBM multi-seed LOSO) but uses XGBoost. Parameters chosen
so that the per-seed search space is comparable in capacity to T11.
...
Outputs (per (seed, fold)):
    loso_pred_xgb_h10_seed{S}_held{K}.parquet  -- prob_0/1/2 + meta
    loso_model_xgb_h10_seed{S}_held{K}.ubj     -- model on disk
"""
...
NUM_CLASS = 3
SYMS = (0, 1, 2, 3, 4)
N_DROP_TAIL = 3  # drop time-encoding tail to match 223-d feature set

# Per-seed diversity-forcing configs (T11 lesson: same hp + diff seed = no improvement).
# Each config varies subsample / colsample / depth / reg_lambda / min_child_weight.
SEED_CONFIGS = {
...
def train_one(H, seed_cfg, held, train_full, val_full, test_full, feat_names, args):
    ...
    params = {
        ...
        "max_depth": int(seed_cfg["max_depth"]),
        "min_child_weight": int(seed_cfg["min_child_weight"]),
        "subsample": float(seed_cfg["subsample"]),
        "colsample_bytree": float(seed_cfg["colsample_bytree"]),
        "reg_lambda": float(seed_cfg["reg_lambda"]),
    ...
    # Predict on test using best_iter
    te_prob = booster.predict(dtest, iteration_range=(0, best_iter + 1)).astype(np.float32)
    ...
    pred_path = os.path.join(HERE, f"loso_pred_xgb_h{H}_seed{seed}_held{held}.parquet")
    te_df.to_parquet(pred_path, index=False)
    model_path = os.path.join(HERE, f"loso_model_xgb_h{H}_seed{seed}_held{held}.ubj")
...
def main():
    ...
    ap.add_argument("--out-tag", default="T18")
    args = ap.parse_args()
    ...
    print(f"=== T18 XGBoost LOSO Scheme C 223-d, horizons={horizons}, seeds={seeds}, syms={target_syms} ===",
          flush=True)
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = f"T18-xgboost-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            init_kwargs = dict(
                project="liangwenbei",
                name=run_name,
                config={
                    ...
                    "horizons": horizons,
                    "seeds": seeds,
                    "diversity_strategy": "per-seed (subsample, colsample_bytree, max_depth, reg_lambda, min_child_weight)",
                    "seed_configs": {str(s): SEED_CONFIGS[s] for s in seeds},
                    **{k: v for k, v in vars(args).items() if k != "no_wandb"},
                },
                tags=["T18", "xgboost", "schemeC", "223d", "loso"],
            )
            try:
    ...
    with open(summary_path, "w") as f:
        json.dump(sanitize_for_json({
            "task": "T18 XGBoost LOSO Scheme C 223-d",
