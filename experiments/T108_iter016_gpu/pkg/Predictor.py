"""Predictor for iter_016 CHIS — GPU torch version.

Identical pipeline to T107 / iter_016 v1, except the T95 GRU pool is
implemented in PyTorch and uses CUDA when available. The numpy GRU is kept
as a robustness fallback (auto-degrade if torch is unavailable or CUDA
init fails). Model weights are still the .npz files shipped in the package
(no .pt dependency added).

Per-horizon stacks are unchanged:
