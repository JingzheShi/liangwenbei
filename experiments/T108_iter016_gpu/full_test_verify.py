"""Verify iter_016 GPU full-442k cum_pnl + GRU timing.

Reuses the cached schemeP_test (442k x 370) + lob_test_top5 (442k x 100 x 20)
caches built in T68 / T95. Loads the same 5x NN + 5x CB + 5x Huber + 5x GRU
ensemble used in T106, but runs the GRU pool through _GRUEnsembleTorch on
CUDA when available.

Reports:
  - cum_pnl total + per-sym (target +43.12 = T106 numpy GRU result)
  - GRU inference time (target: < 60s vs T106's 1391s numpy)
...
def main():
    print("=== iter_016 GPU torch-GRU full-test verification ===", flush=True)
    ...
    for s in SEEDS:
        ...
        cb.load_model(os.path.join(PKG, f"model_T89_seed{s}.cbm"))
        cb_models.append(cb)
    ...
    p_cb /= len(cb_models)
    t_cb = time.time() - t0
    print(f"  T89 CB inference in {t_cb:.1f}s  mean={p_cb.mean():+.3e} std={p_cb.std():.3e}", flush=True)
    ...
    if gru_ens is not None:
        ...
        # legacy numpy fallback (very slow)
    ...
    print(f"  vs T106 numpy GRU baseline +43.1218: diff={sum_per_sym-43.1218:+.6f}", flush=True)
    ...
    out = {
        ...
        "vs_T106_numpy": sum_per_sym - 43.1218,
    }
