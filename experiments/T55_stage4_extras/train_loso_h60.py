"""T55 LOSO h=60 LightGBM training on schemeP cache (226 + 54 + 59 + 14 + 47 = 400 dim).

Same recipe as T53 but with stage 4 added:
...
def main():
    ...
    ap.add_argument("--out-tag", default="r34_stage4")
    ...
    print(f"=== T55 LOSO h={H} variant={args.variant} seeds={seeds} syms={syms} ===", flush=True)
    ...
    n_extras = total_dim - base_dim  # 174 (54+59+14+47)
    extras_names_path = os.path.join(CACHE_DIR, "schemeP_extra_feat_names.txt")
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = f"T55-LOSO-h{H}-{args.variant}-{args.out_tag}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            wandb.init(
                project="liangwenbei",
                ...
                tags=["T55", "R34-stage4", "LOSO", f"h{H}"],
            )
    ...
    summary = {
        "task": "T55 R34 stage 4 features LOSO",
        "params": vars(args),
        "horizon": H,
