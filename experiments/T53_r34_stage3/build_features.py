"""T53 R34 Stage 3 feature builder.

Reads each session parquet, computes:
  - T51 R34 stage 1 (54 dims) — re-using r34_features.compute_all_session
  - T51 R34 stage 2 (59 dims) — r34_stage2_features.compute_all_session
  - T53 R34 stage 3 (14 dims) — r34_stage3_features.compute_all_session
Aligns to the existing schemeC cache row order, then concatenates:
  [226 schemeC | 54 stage1 | 59 stage2 | 14 stage3]  -> 353 dims (schemeN)

Output:
    cache/schemeN_train.npz / schemeN_val.npz / schemeN_test.npz
    cache/schemeN_extra_feat_names.txt  (54+59+14 = 127 stage extras)
    cache/schemeN_feat_names.txt        (353 names)
...
def build_split(sym_dates, cache_dir: str) -> dict:
    ...
    for i, (sym, date, sess) in enumerate(sym_dates):
        ...
        if not np.all(np.isfinite(f3)):
            f3 = np.where(np.isfinite(f3), f3, 0.0).astype(np.float32)
    ...
    print(
        ...
        f"extra3 {out['X_extra3'].shape} {out['X_extra3'].nbytes/1e6:.2f}MB",
        flush=True,
    )
    return out
...
def merge_and_save(extra: dict, scheme_c_npz: str, out_npz: str) -> None:
    """Concat schemeC X with extra1 + extra2 + extra3 and save under schemeN_{split}.npz."""
...
def main():
    ...
    print(f"Stage 1: {len(stage1_names)} | Stage 2: {len(stage2_names)} | Stage 3: {len(stage3_names)}",
          flush=True)
    ...
    extra_names = stage1_names + stage2_names + stage3_names
    full_names = base_names + extra_names
    print(
        f"schemeN: {len(full_names)} = {len(base_names)} + {len(stage1_names)} + "
        f"{len(stage2_names)} + {len(stage3_names)}",
        flush=True,
    )

    extra_path = os.path.join(args.out_dir, "schemeN_extra_feat_names.txt")
    full_path = os.path.join(args.out_dir, "schemeN_feat_names.txt")
    ...
    for key in wanted:
        ...
        n_path = os.path.join(args.out_dir, f"schemeN_{key}.npz")
