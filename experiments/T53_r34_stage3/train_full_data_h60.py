"""T53b full-data h=60 5-seed train on schemeN cache (340-d) for iter_008 packaging.

Mirrors T51 train_full_data_h60.py but for schemeN (Stage 1+2+3, 340-d after drop).
For each seed in {1, 7, 13, 42, 100}:
  - X = train + val concat
  - aug_a applied on the concatenated set
  - num_boost_round fixed (no early stopping)
...
def build_keep_idx():
    """Same drop list as LOSO training: 10 FAIL extras + 3 time-encoding tail."""
    base_dim = 226
...
def main():
    ...
    ap.add_argument("--num-boost-round", type=int, default=300,
                    help="Fixed iterations. T53 LOSO best_iter mean ~138 -> use 300 for full data.")
    ...
    print(f"=== T53b full-data h={H} seeds={seeds} ===", flush=True)
    train_full = load_split("train")
    val_full = load_split("val")

    keep, fail_names = build_keep_idx()
    print(f"  keep_dim={len(keep)} dropped {len(fail_names)} FAIL + 3 tail")
    ...
    summary = {"task": "T53b full-data h60 5-seed (schemeN 340-d)", "n_full": int(len(y_full)),
