"""Predictor for T190: 150 NN + 150 LGB mega ensemble with optimized batched inference.

T190 extends T188v2 (50+50) to 150+150. Seeds 51-150 may have different hidden sizes
(256/384/320/512/192), so _BatchedMLPEnsemble is now wrapped in _HeterogeneousBatchedEnsemble
that groups models by architecture and runs one bmm-batched sub-ensemble per unique
architecture, then averages predictions weighted by group size.

  pred_lgb = mean over up to 150 LightGBM regression boosters
  pred_nn  = weighted-count mean over up to 150 MLPs (grouped by architecture)
  pred     = (w_nn * pred_nn + w_lgb * pred_lgb) / (w_nn + w_lgb), w_nn=1.0, w_lgb=1.5

per-sym calibrated abstain band (inherited from T188v2):
...
Compliance with CRITICAL_CONSTRAINTS.md:
  - sym used only for per-sym beta lookup; not fed to model.forward
  - date never used; no cross-call state; each predict() is fully independent
  - sym-agnostic FORWARD: global standardization (no per-sym); W <= 100; stateless
"""
...
class _BatchedMLPEnsemble:
    ...
    @torch.no_grad()
    def predict_sum(self, X_in: np.ndarray) -> np.ndarray:
        """Batched forward for all N NNs; returns SUM (not mean) over N, shape (B,)."""
        ...
        return out.sum(0).cpu().numpy().astype(np.float32)  # SUM over N
...
class _HeterogeneousBatchedEnsemble:
    ...
    Groups models by (hidden_tuple) and creates one _BatchedMLPEnsemble per group.
    ...
    @torch.no_grad()
    def predict_mean(self, X_in: np.ndarray) -> np.ndarray:
        if len(self._sub_ensembles) == 1:
            return self._sub_ensembles[0].predict_mean(X_in)
        acc = None
        for sub, sz in zip(self._sub_ensembles, self._sub_sizes):
...
if __name__ == "__main__":
    ...
    p = Predictor()
    load_t = time.time() - t0
    print(f"T190 Predictor loaded in {load_t:.1f}s")
    print(f"Device: {p._device}")
    n_lgb = len(p._lgb_lists.get(60, []))
    ens60 = p._nn_batched.get(60)
    n_nn = sum(s.N for s in ens60._sub_ensembles) if ens60 else 0
    n_nn_groups = len(ens60._sub_ensembles) if ens60 else 0
    print(f"Loaded: {n_lgb} LGB, {n_nn} NN ({n_nn_groups} arch groups), device={p._device}")
    t1 = time.time()
    out = p.predict([df, df, df])
    inf_t = time.time() - t1
    print(f"smoke test predict -> {out[0]}  inf_time={inf_t*1000:.1f}ms for 3 batches")
    df_ood = df.copy()
    df_ood["sym"] = 99
    out_ood = p.predict([df_ood])
    print(f"OOD sym=99 -> {out_ood[0]}")
    df_no_sym = df.drop(columns=["sym"])
    out_no = p.predict([df_no_sym])
    print(f"missing sym -> {out_no[0]}")
