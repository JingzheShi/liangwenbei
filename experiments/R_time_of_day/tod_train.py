"""TOD trick: LGB Huber alpha=0.001 on schemeP 359 + 5 time-of-day features.

Same training pipeline as R4 (T75-style baseline) but adds 5 TOD features
when --use-tod is set. Outputs to /root/lwb_work_tod/.

Usage:
  python3 tod_train.py --seed 42                # baseline (359-d)
  python3 tod_train.py --seed 42 --use-tod      # trick (359+5-d)
"""
...
def load_tod(split):
    p = os.path.join(TOD_DIR, f"tod_{split}.npz")
    d = np.load(p, allow_pickle=True)
    return d["tod"].astype(np.float32, copy=False), d["feat_names"].tolist()
...
def build_v4_with_tod(train_full, slicer, tod_train=None):
    ...
    if tod_train is not None:
        X_tr = np.concatenate([Xb_tr, tod_train[m_t]], axis=1)
        X_va = np.concatenate([Xb_va, tod_train[m_va]], axis=1)
    else:
    ...
    return (X_tr, y60_tr, yreg_tr, sym_tr, X_va, y60_va, yreg_va, sym_va)


def train_lgb(X_tr, yreg_tr, sw_tr, X_va, yreg_va, sw_va,
              feat_names, cfg, seed,
              num_boost_round=300, early_stopping=25, lr=0.05,
              alpha=0.001, verbose_period=200):
...
def main():
    ...
    ap.add_argument("--use-tod", action="store_true")
    ...
    tag = "trick" if use_tod else "baseline"
    print(f"=== TOD LGB Huber alpha={args.alpha} seed={seed} use_tod={use_tod} ({tag.upper()}) ===", flush=True)
    t_total0 = time.time()
    ...
    if use_tod:
        tod_train, tod_names = load_tod("train")
        tod_test, tod_names_te = load_tod("test")
        assert tod_names == tod_names_te
        assert tod_train.shape[0] == train_full["X"].shape[0]
        assert tod_test.shape[0] == test_full["X"].shape[0]
        feat_names_full = feat_names + list(tod_names)
        print(f"  feat_dim base={len(keep)} tod={tod_train.shape[1]} total={len(feat_names_full)}", flush=True)
    else:
        tod_train = tod_test = None
        feat_names_full = feat_names
        print(f"  feat_dim={len(feat_names_full)}", flush=True)

    (X_tr, y60_tr, yreg_tr, sym_tr,
     X_va, y60_va, yreg_va, sym_va) = build_v4_with_tod(train_full, keep, tod_train)
    ...
    if use_tod:
        X_te = np.concatenate([Xb_te, tod_test], axis=1)
    else:
        X_te = Xb_te
    ...
    print(f"  training LGB GPU (use_tod={use_tod})...", flush=True)
    t0 = time.time()
    ...
    model_path = os.path.join(OUT_DIR, f"model_TOD_{tag}_seed{seed}.txt")
    ...
    pred_path = os.path.join(OUT_DIR, f"pred_TOD_{tag}_seed{seed}.parquet")
    ...
    summary = {
        "task": f"TOD LGB Huber seed={seed} use_tod={use_tod}",
        "seed": seed, "use_tod": use_tod, "tag": tag,
        "alpha": args.alpha,
        "best_iter": best_iter,
    ...
    out_path = os.path.join(OUT_DIR, f"summary_TOD_{tag}_seed{seed}.json")
