"""R_Hawkes_OFI: 3-seed ensemble × 2 variants DE evaluation.

Per ensemble:
  1. Average the 3-seed predictions
  2. DE 2D thresh (T_up, T_dn shared across all syms/sessions) → LOSO-equiv (sum per-sym)
  3. DE 4D thresh (T_up_am, T_dn_am, T_up_pm, T_dn_pm) per session → LOSO-equiv
  4. Per-sym PnL with both gates

Outputs: results.json with baseline / hawkes / delta.
"""
...
def gate_4d_session(p, sess_arr, tu_am, td_am, tu_pm, td_pm):
    """Session-split asym thresh: 0=am, 1=pm. Equivalent to gate_2d with
    (tu, td) per session_idx."""
    a = np.full(len(p), 1, dtype=np.int8)
    is_am = (sess_arr == 0)
    is_pm = ~is_am
    a[is_am & (p > tu_am)] = 2
...
def fit_de_4d(p_arr, sym_arr, sess_arr, mp_t, mp_th, de_seeds=(0, 42, 1)):
    """4D: (tu_am, td_am, tu_pm, td_pm). Sum LOSO per-sym."""
...
def evaluate_arm(name, paths):
    ...
    sess_arr = (base["session"].to_numpy() == "pm").astype(np.int8)  # 0=am, 1=pm
    ...
    print(f"  DE 2D:  tu={tu:.4e} td={td:.4e} loso={tot_2d:.4f}  ({time.time()-t0:.1f}s)",
          flush=True)
    ...
    print(f"  DE 4D:  am=({tu_am:.3e},{td_am:.3e}) pm=({tu_pm:.3e},{td_pm:.3e}) loso={tot_4d:.4f}  ({time.time()-t0:.1f}s)",
          flush=True)
...
def main():
    print("=== R_Hawkes_OFI 3-seed × 2-variant DE eval ===", flush=True)
    arms = {
        "baseline":  [os.path.join(HERE, f"pred_baseline_seed{s}.parquet") for s in SEEDS],
        "hawkes":    [os.path.join(HERE, f"pred_hawkes_seed{s}.parquet")   for s in SEEDS],
    }
    # Verify all exist
    ...
    base_2d = results["baseline"]["de_2d"]["loso_equiv"]
    base_4d = results["baseline"]["de_4d_session"]["loso_equiv"]
    trk_2d = results["hawkes"]["de_2d"]["loso_equiv"]
    trk_4d = results["hawkes"]["de_4d_session"]["loso_equiv"]

    print("\n=== Summary ===", flush=True)
    print(f"  Baseline DE 2D LOSO:  {base_2d:+.4f}", flush=True)
    print(f"  Baseline DE 4D LOSO:  {base_4d:+.4f}", flush=True)
    print(f"  +Hawkes  DE 2D LOSO:  {trk_2d:+.4f}  (Δ = {trk_2d - base_2d:+.4f})", flush=True)
    print(f"  +Hawkes  DE 4D LOSO:  {trk_4d:+.4f}  (Δ = {trk_4d - base_4d:+.4f})", flush=True)

    # Per-sym min delta (key for non-overfit guarantee)
    base_per = results["baseline"]["de_4d_session"]["per_sym"]
    trk_per = results["hawkes"]["de_4d_session"]["per_sym"]
    ...
    out = {
        "arms": results,
        "delta_2d": trk_2d - base_2d,
        "delta_4d": trk_4d - base_4d,
