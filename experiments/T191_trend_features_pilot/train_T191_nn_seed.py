"""T191 NN trainer: same T188v2 protocol (T81-pretrain + T170 SPO+ finetune),
but features = 370-d schemeP ⊕ 5-d trend = 375-d (after DROP_NAMES -> 364).

Seeds used: {1, 7, 13, 42, 100} matching T170 baseline.
Saves model_npz and pt to local HERE/ directory.
"""
...
# CACHE_DIR_SP = scheme P 370-d original cache
# CACHE_DIR_TR = trend 5-d cache (this dir / cache)
...
def progress(step, **extra):
    p = os.path.join(HERE, f"worker-progress.json")
    payload = {
...
def load_combined(split: str):
    """Returns dict with X (N, 375), mp_t, mp_t<h>, y<h>, sym, date."""
    sp_path = os.path.join(CACHE_DIR_SP, f"schemeP_{split}.npz")
    tr_path = os.path.join(CACHE_DIR_TR, f"trend_feat_{split}.npz")
    print(f"  loading {sp_path}", flush=True)
    d = np.load(sp_path)
    out = {k: d[k] for k in d.files}
    d.close()
    print(f"  loading {tr_path}", flush=True)
    dt = np.load(tr_path)
    T = dt["T"].astype(np.float32)
    dt.close()
    assert len(T) == len(out["X"]), f"alignment fail {split}: T={len(T)} X={len(out['X'])}"
    out["X"] = np.concatenate([out["X"], T], axis=1).astype(np.float32)
    return out
...
def main():
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"T191 NN seed={S} h={H} lr_p1={lr_p1} dropout={dropout} batch_p1={batch_p1}", flush=True)
    print(f"device={device}  CACHE_SP={CACHE_DIR_SP}  CACHE_TR={CACHE_DIR_TR}", flush=True)
    torch.manual_seed(S); torch.cuda.manual_seed_all(S); np.random.seed(S)

    # Build feature names: 370 schemeP + 5 trend = 375
    ...
    print(f"  feat_dim={feat_dim} (370+5 - {len(drop_idx)} dropped)", flush=True)
    assert feat_dim == 364, f"expected 364 got {feat_dim}"

    anchor_path = os.path.join(HERE, "T81_pretrained_T191", f"anchor_seed{S}.pt")
    pt_path = os.path.join(HERE, f"nn_h60_seed{S}_T191.pt")
    npz_path = os.path.join(HERE, f"nn_h60_seed{S}_T191.npz")
    ...
    scales = rng_aug.uniform(AUG_LO, AUG_HI, size=X_tr.shape).astype(np.float32)
    X_aug = X_tr * scales
    ...
    best_val_mse = float("inf"); best_epoch = -1; best_state = None; bad_epochs = 0
    n_train = len(X_tr_raw); t0_p1 = time.time()
    for epoch in range(PHASE1_EPOCHS):
        ...
        for s in range(0, n_train, batch_p1):
            ...
            optimizer.zero_grad(); mse.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 5.0)
            optimizer.step()
            r_loss += float(mse.item()) * xb.size(0); r_n += xb.size(0)
        scheduler.step()
        ...
        print(f"  p1 ep {epoch:2d}  train={r_loss/r_n:.4e}  val_mse={val_mse:.4e}  corr={va_corr:.4f}", flush=True)
    ...
    print(f"  Phase 1 done {t_p1:.1f}s  best_ep={best_epoch}  best_val_mse={best_val_mse:.4e}", flush=True)
    assert best_state is not None

    ckpt_p1 = {
        "state_dict": best_state, "feat_mean": feat_mean, "feat_std": feat_std,
        "keep_idx": keep_idx, "feat_names": feat_names, "hidden": HIDDEN,
        "dropout": dropout, "use_layernorm": True, "in_dim": feat_dim,
        "target_scale": float(target_scale), "clip": float(CLIP),
        "best_epoch": best_epoch, "best_val_mse": best_val_mse,
        "phase1_lr": lr_p1, "phase1_batch": batch_p1, "seed": S,
    }
    torch.save(ckpt_p1, anchor_path)

    # --- Phase 2: SPO+ M7 ---
    print(f"\n--- Phase 2: SPO+ M7 seed={S} ---", flush=True)
    progress("phase2_loading", seed=S)
    train_d = load_combined("train"); val_d = load_combined("val"); test_d = load_combined("test")
    ...
    n_orig = len(X_all)
    X_full2 = np.empty((2 * n_orig, X_all.shape[1]), dtype=np.float32)
    X_full2[:n_orig] = X_all
    CHUNK = 200_000
    for i in range(0, n_orig, CHUNK):
        j = min(i + CHUNK, n_orig)
        sc = rng_aug2.uniform(AUG_LO, AUG_HI, size=(j - i, X_all.shape[1])).astype(np.float32)
        X_full2[n_orig + i:n_orig + j] = X_all[i:j] * sc
    del X_all
    ...
    X_tr_raw = torch.from_numpy(X_full2.astype(np.float32))
    ...
    n_train2 = len(X_tr_raw); t0_p2 = time.time()
    for epoch in range(PHASE2_EPOCHS):
        ...
        for s in range(0, n_train2, PHASE2_BATCH):
            ...
            torch.nn.utils.clip_grad_norm_(model.parameters(), 5.0)
            optimizer.step()
            bs = xb.size(0); r_l2 += float(l2_loss.item()) * bs; r_spo += float(spo_loss.item()) * bs; r_n += bs
        scheduler.step()
        print(f"  p2 ep {epoch:2d}  l2={r_l2/r_n:.4e}  spo={r_spo/r_n:.4e}", flush=True)
    t_p2 = time.time() - t0_p2
    print(f"  Phase 2 done {t_p2:.1f}s", flush=True)
    ...
    final_ckpt = {
        ...
        "dropout": dropout, "use_layernorm": True, "in_dim": feat_dim,
        "target_scale": float(target_scale), "clip": float(CLIP),
        "epochs_fixed": PHASE2_EPOCHS, "lambda_spo": PHASE2_LAMBDA_SPO,
        "M7_full_retrain": True, "data_dates": "0-119",
        "phase1_best_epoch": int(ckpt_p1["best_epoch"]), "seed": S,
    }
    ...
    print(f"DONE NN seed={S} t_p1={t_p1:.1f} t_p2={t_p2:.1f}", flush=True)
