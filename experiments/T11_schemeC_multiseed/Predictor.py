"""Predictor for iter_004: 5-seed LightGBM ensemble, Scheme C 223-d, multi-horizon.
...
Difference from iter_002:
  Each active horizon has K seed models (model_h{H}_seed{S}.txt). At inference
  we predict probabilities with each, average them, then threshold.

Bundle:
  thresholds.json            seeds list + per-horizon (T, delta) and active flag
  model_h{H}_seed{S}.txt     LightGBM booster, 223-d input, one per (H, S)
"""
...
def _load_t3_module(here: str):
    spec = importlib.util.spec_from_file_location(
        "iter_004_t3_compute", os.path.join(here, "compute.py")
    )
...
class Predictor:
    def __init__(self) -> None:
        ...
        self._seeds: List[int] = list(tcfg.get("seeds", [42]))

        # Load all (H, seed) boosters into memory once.
        # boosters[H] = list of boosters across seeds (in same order as self._seeds).
    ...
    def predict(self, batches: List[pd.DataFrame]) -> List[List[int]]:
        ...
        for hcfg in self._horizons:
            ...
            if not ms:
                continue
            # Average prob across seeds.
            prob_sum = np.zeros((B, 3), dtype=np.float32)
            for booster in ms:
                prob_sum += booster.predict(feats).astype(np.float32)
