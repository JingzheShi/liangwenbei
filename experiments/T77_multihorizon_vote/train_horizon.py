"""T77: Train horizon-specific LightGBM model with V4 split + Stage 5 features.

Adapted from T70 train_v4_stage5.py to use y{H} (instead of hardcoded y60) for training,
val, and test, so we can produce h_5/h_10/h_20/h_40 predictions consistent with T70 h_60.
"""
...
def build_v4_split(train_full, slicer, H):
    ...
    Uses y{H} for labels so model is horizon-specific.
    """
    ...
    y_tr = train_full[yk][m_t].astype(np.int64)
    X_va = train_full["X"][m_va][:, slicer].astype(np.float32, copy=False)
    y_va = train_full[yk][m_va].astype(np.int64)
    mp_t_va = train_full["mp_t"][m_va]
    mp_th_va = train_full[mpk][m_va]
    info = {
        "strategy": "V4",
        "horizon": H,
        "train_dates": "0-75",
...
def main():
    ...
    ap.add_argument("--out-tag", default="t77")
    ap.add_argument("--no-wandb", action="store_true")
    args = ap.parse_args()

    H = args.horizon
    seed = args.seed
    print(f"=== T77 horizon={H} seed={seed} ===", flush=True)
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = (f"T77-h{H}-seed{seed}-{args.out_tag}-"
    ...
    seed_rng = np.random.default_rng(seed * 7919 + 1 + H)
    ...
    model_path = os.path.join(HERE, f"model_T77_h{H}_seed{seed}.txt")
    ...
    va_metrics = evaluate(f"VAL h={H} seed={seed}", va_pred, y_va, mp_t_va, mp_th_va)
    ...
    pred_path = os.path.join(HERE, f"pred_T77_h{H}_seed{seed}.parquet")
    te_df.to_parquet(pred_path, index=False)
    print(f"  saved {pred_path} ({len(te_df):,} rows)", flush=True)

    summary = {
        "task": f"T77 horizon={H} seed={seed}",
        "seed": seed,
        "horizon": H,
        "split_info": info,
        "n_features": feat_dim,
    ...
    out_path = os.path.join(HERE, f"summary_T77_h{H}_seed{seed}.json")
