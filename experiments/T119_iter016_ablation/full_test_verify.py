"""Verify T119 iter_016 ablation full-442k cum_pnl on the cached test set.

Reuses the cached schemeP_test (442k x 370) cache built in T68.
Loads only the 3 non-GRU streams used by the ablation pkg:

  - 5x T87 SPO+ NN ensemble (nn_h60_seed*.npz)
  - 5x T99 CB Huber alpha=1e-3 ensemble (model_cb_huber_seed*.cbm)
  - 5x T99 LGB Huber alpha=1e-3 ensemble (model_T99_huber_a0.001_seed*.txt)

Combines with weights w=(1.0, 0.7, 1.0) for (NN, CB_Huber, LGB_Huber)
and applies conservative thresh thr_up=4.21e-4 / thr_dn=1.86e-4.
Short horizons are disabled (always action=1), so only h=60 contributes
non-trivial cum_pnl in this verify.
"""
...
def main():
    print("=== T119 iter_016 ablation full-test verification ===", flush=True)
    ...
    # references
    ref_v3 = 44.7178       # T109 v3 4-way (with GRU + aggressive thr)
    ref_iter015_v1 = 40.13  # iter_015 v1 packaged
    ref_iter015_v2 = 43.20  # iter_015 v2 / T106

    print(f"  vs T109 v3 (+44.72 LOSO-equiv): diff={sum_per_sym-ref_v3:+.4f}", flush=True)
    print(f"  vs iter_015 v1 (+40.13 packaged): diff={sum_per_sym-ref_iter015_v1:+.4f}", flush=True)
    print(f"  vs iter_015 v2 (+43.20 packaged): diff={sum_per_sym-ref_iter015_v2:+.4f}", flush=True)

    out = {
        "iter": "T119 iter_016 ablation (no GRU, conservative thr, short horizons disabled)",
        ...
        "weights": {"nn": w_nn, "cb_huber": w_cb, "lgb_huber": w_huber, "gru": 0.0},
        "timing_seconds": {
            "T87_nn": t_nn,
            "T99_cb_huber": t_cb,
        ...
        "vs_T109_v3": sum_per_sym - ref_v3,
        "vs_iter015_v1": sum_per_sym - ref_iter015_v1,
        "vs_iter015_v2": sum_per_sym - ref_iter015_v2,
    }
