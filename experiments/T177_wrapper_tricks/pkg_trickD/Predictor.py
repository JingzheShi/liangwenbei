"""Predictor for iter_018 v1 (T127): iter_015 v1 stack + per-sym beta abstain wrapper.
...
class Predictor:
    def __init__(self) -> None:
        ...
        self._cw_per_sym_temp: Dict[int, float] = {}
        self._cw_per_sym_thr_factor: Dict[int, float] = {}
        if self._cw_enabled:
            ...
            # T177 Trick B: per-sym temperature for combined pred_dmid
            pst = cw.get("per_sym_temp", {})
            self._cw_per_sym_temp = {int(k): float(v) for k, v in pst.items()}
            # T177 Trick C: per-sym thr_factor (scales thr_up and thr_dn before adding band)
            pstf = cw.get("per_sym_thr_factor", {})
            self._cw_per_sym_thr_factor = {int(k): float(v) for k, v in pstf.items()}
    ...
    def _gate_with_band(self, pred_dmid: np.ndarray, hcfg: Dict,
                        band_per_row: np.ndarray,
                        thr_factor_per_row: np.ndarray | None = None) -> np.ndarray:
        thr_up = float(hcfg.get("thr_up", 2.0e-4))
        thr_dn = float(hcfg.get("thr_dn", 2.0e-4))
        # T177 Trick C: scale thresholds per sym before adding conformal band
        if thr_factor_per_row is not None:
            eff_up = thr_up * thr_factor_per_row + band_per_row
            eff_dn = thr_dn * thr_factor_per_row + band_per_row
        else:
            eff_up = thr_up + band_per_row
            eff_dn = thr_dn + band_per_row
        out = np.full(pred_dmid.shape[0], 1, dtype=np.int64)
        out[pred_dmid > eff_up] = 2
        out[pred_dmid < -eff_dn] = 0
        return out
    ...
    def _extract_temp_per_row(self, batches: List[pd.DataFrame]) -> np.ndarray:
        """Look up per-sym temperature factor (T177 Trick B). OOD/missing -> 1.0."""
        B = len(batches)
        ...
        for i, df in enumerate(batches):
            ...
            try:
                ...
                continue
            if s in self._cw_per_sym_temp:
                out[i] = self._cw_per_sym_temp[s]
        return out

    def _extract_thr_factor_per_row(self, batches: List[pd.DataFrame]) -> np.ndarray:
        """Look up per-sym threshold factor (T177 Trick C). OOD/missing -> 1.0."""
        B = len(batches)
        out = np.ones(B, dtype=np.float64)
        for i, df in enumerate(batches):
            if "sym" not in df.columns:
            ...
            try:
                ...
                continue
            if s in self._cw_per_sym_thr_factor:
                out[i] = self._cw_per_sym_thr_factor[s]
        return out

    @staticmethod
    def _ensemble_predict_lgb(boosters: List[lgb.Booster], X: np.ndarray) -> np.ndarray:
        # T177 Trick A: median across seeds
        if len(boosters) == 1:
            return boosters[0].predict(X).astype(np.float32, copy=False)
        preds = np.stack([b.predict(X).astype(np.float32) for b in boosters], axis=0)
    ...
    def _ensemble_predict_nn(nns: List[_MLPNumpy], X: np.ndarray) -> np.ndarray:
        ...
        preds = np.stack([n.predict(X) for n in nns], axis=0)
        return np.median(preds, axis=0).astype(np.float32)
    ...
    def predict(self, batches: List[pd.DataFrame]) -> List[List[int]]:
        ...
        if self._cw_enabled:
            ...
            temp_per_row = self._extract_temp_per_row(batches) if self._cw_per_sym_temp else None
            thr_factor_per_row = self._extract_thr_factor_per_row(batches) if self._cw_per_sym_thr_factor else None
        else:
            band_per_row = np.zeros(B, dtype=np.float64)
            temp_per_row = None
            thr_factor_per_row = None
        ...
        for hcfg in self._horizons:
            ...
            # T177 Trick B: apply per-sym temperature to combined prediction
            if temp_per_row is not None:
                pred_dmid = pred_dmid * temp_per_row

            if self._cw_enabled:
                # T177 Trick C: pass per-sym thr_factor to gate
                actions = self._gate_with_band(pred_dmid, hcfg, band_per_row, thr_factor_per_row)
            else:
