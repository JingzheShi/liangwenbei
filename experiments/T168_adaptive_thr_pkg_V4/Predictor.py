"""Predictor V4_adaptive_thr: T168 per-batch hybrid adaptive threshold variant.

V4: use adaptive threshold only for extreme batches (ratio >= 2x or <= 0.5x baseline).
Normal batches use original static thresholds. Strong batches → lower thresholds;
weak batches → higher thresholds (capped at 3x). Conformal band applied on top.
...
class Predictor:
    ...
    def predict(self, batches: List[pd.DataFrame]) -> List[List[int]]:
        ...
        for hcfg in self._horizons:
            ...
            # V4: hybrid — use adaptive thresh only for extreme batches
