"""T74: V4 walk-forward val + Stage 5 + 6 time features (schemeQ cache).

Identical pipeline to T70 V4 stage5 EXCEPT we read schemeQ_*.npz which appends
6 time features (sec_norm, sin_sess_pos, cos_sess_pos, is_open_30min,
is_close_30min, is_pm) to the schemeP 370-d feature vector.

Drop set is the same as T70 (T59 sym-invariance fail + Stage 5 fail = 11 names).
The 6 time features are kept (sym-invariant by construction).

5-seed full-data, GPU LightGBM, num_boost=600, early_stop=40, aug_a [0.80, 1.20].
Final feat_dim = 376 - 11 = 365.
"""
...
# Drop set: T59 fail + Stage 5 fail (time features kept)
T59_FAIL_NAMES = [
...
def load_split(split):
    p = os.path.join(CACHE_DIR, f"schemeQ_{split}.npz")
...
def main():
    ...
    ap.add_argument("--out-tag", default="v4_time")
    ap.add_argument("--no-wandb", action="store_true")
    args = ap.parse_args()

    H = args.horizon
    seed = args.seed
    print(f"=== T74 V4 + Stage 5 + Time seed={seed} h={H} ===", flush=True)
    ...
    feat_names_path = os.path.join(CACHE_DIR, "schemeQ_feat_names.txt")
    ...
    X_tr, y_tr, X_va, y_va, mp_t_va, mp_th_va, info = build_v4_split(train_full, slicer)
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = (f"T74-V4-time-seed{seed}-{args.out_tag}-"
            ...
            wandb.init(
                ...
                tags=["T74", "V4", "time_features", f"h{H}"],
            )
    ...
    model_path = os.path.join(HERE, f"model_T74_seed{seed}.txt")
    ...
    va_metrics = evaluate(f"VAL V4 seed={seed}", va_pred, y_va, mp_t_va, mp_th_va)

    te_prob = predict_proba(booster, X_te)
    te_pred = te_prob.argmax(axis=1).astype(np.int8)
    te_metrics = evaluate(f"TEST full 5-sym seed={seed}",
    ...
    pred_path = os.path.join(HERE, f"pred_T74_seed{seed}.parquet")
    te_df.to_parquet(pred_path, index=False)
    print(f"  saved {pred_path} ({len(te_df):,} rows)", flush=True)

    summary = {
        "task": f"T74 V4 time seed={seed}",
        "seed": seed,
        "horizon": H,
        "split_info": info,
        "n_features": feat_dim,
    ...
    out_path = os.path.join(HERE, f"summary_T74_seed{seed}.json")
