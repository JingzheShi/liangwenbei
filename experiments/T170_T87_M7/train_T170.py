"""T170: T87 SPO+ NN with M7 trick — full data retrain (date 0-119), fixed epochs.

Key difference from T87:
  - Training data: ALL three splits (train 0-79 + val 80-95 + test 96-119)
  - No validation holdout, no early stopping
  - Fixed epochs = 11 (avg T87 best_epoch 9.2 × 1.1, rounded up)
  - Everything else identical to T87 main tag

Average best_epochs from T87 main: seed1=11, seed7=13, seed13=11, seed42=6, seed100=5 → avg=9.2 → ×1.1=10.12 → 11
"""
...
def extract_npz(pt_path, npz_path, ckpt=None):
    """Extract .pt weights to .npz for torch-free Predictor inference."""
    if ckpt is None:
...
def main():
    ...
    ap.add_argument("--epochs", type=int, default=11,
                    help="Fixed epochs (no early stop). avg T87 best=9.2 × 1.1 = 10.12 → 11")
    ...
    H = args.horizon
    seed = args.seed
    print(f"=== T170 M7 full-retrain seed={seed} h={H} lambda_spo={args.lambda_spo} "
    ...
    progress("loading_t81_init", seed=seed)
    ...
    # Aug: concat scheme (double the data with random feature scaling)
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = f"T170-M7-seed{seed}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            wandb.init(
                project="liangwenbei",
                ...
                tags=["T170", "M7", "SPO+", "full-retrain", f"seed{seed}"],
            )
    ...
    summary = {
        "task": f"T170 M7 full-retrain seed={seed}",
        "seed": seed,
        "horizon": H,
        "lambda_spo": args.lambda_spo,
        "M7_full_retrain": True,
    ...
    out_path = os.path.join(HERE, f"summary_T170_seed{seed}.json")
