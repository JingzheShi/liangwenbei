"""T68 DE 4D threshold search on schemeP LOSO h_60 OOF predictions.

Uses T53 de_thresh helpers but local-prefixed to T68 dir.
"""
...
def load_fold_avg_local(prefix: str, k: int, seeds):
    """Load LOSO predictions stored in T68 HERE."""
...
def main():
    ...
    ap.add_argument("--out", default="de_results_seed42.json")
    args = ap.parse_args()

    seeds = [int(s) for s in args.seeds.split(",")]
    print(f"=== T68 DE thresh prefix={args.prefix} seeds={seeds} ===", flush=True)

    t0 = time.time()
    folds = {k: load_fold_avg_local(args.prefix, k, seeds) for k in range(N_FOLDS)}
    fas = fold_arrays_from_dfs(folds)
    n_total = sum(len(f) for f in folds.values())
    ...
    print(coarse_df.head(10)[["T_up", "T_dn", "d_up", "d_dn",
                              "sum_cum_pnl", "n_pos_folds"]].to_string(index=False))
