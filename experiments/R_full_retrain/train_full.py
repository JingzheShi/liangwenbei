"""R_full_retrain: T75 LGB L2 retrained on FULL date 0-119 (train+val+test), no early stop.

Meta-paradigm M7: standard Kaggle final-submit trick.
- Same hyperparams + per-seed configs as T75_regression_dmid.
- num_boost_round fixed = round(avg(T75 best_iter) * 1.1) ≈ 330.
...
def main():
    ...
    H = args.horizon
    seed = args.seed
    print(f"=== R_full_retrain seed={seed} h={H} num_boost={args.num_boost_round} ===", flush=True)
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = (f"Rfullretrain-seed{seed}-{args.objective}-"
            ...
            wandb.init(
                ...
                tags=["R_full_retrain", "M7", f"h{H}", "no_es", f"nbr{args.num_boost_round}"],
            )
    ...
    summary = {
        "task": f"R_full_retrain seed={seed}",
        "seed": seed,
        "horizon": H,
        "objective": args.objective,
        "n_features": feat_dim,
