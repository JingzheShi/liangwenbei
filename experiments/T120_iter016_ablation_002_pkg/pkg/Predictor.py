"""Predictor for iter_016 ablation 002 — T119 base + LGB Huber monotone constraints.

Ablation 002 rationale:
  ablation_001 (T119) = T87 NN + T99 CB Huber + T99 LGB Huber, no GRU,
  conservative thresh. Local LOSO +39.98.
  ablation_002 swaps the LGB Huber stream for T117's monotone-constrained
  re-train (18 microstructure features with theory-signed +/-1 monotone
  constraints, advanced method). On the 3-seed LOSO bake-off T117 monotone
  beat T99 baseline by +1.87. Same conservative thresholds + same NN/CB/LGB
  weights so the only delta vs ablation_001 is monotone training.
        ...
        for hcfg in self._horizons:
            ...
            if kind == "lgb_cb_2way":
                ...
                for s in seeds:
                    ...
                    hu_p = os.path.join(here, f"model_T120_huber_mono_seed{s}.txt")
