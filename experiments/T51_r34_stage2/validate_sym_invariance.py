"""Sym-invariance KS test for T51 schemeM cache (54 stage1 + 59 stage2 = 113 extras).
...
def main():
    ...
    n_extra = len(names)
    print(f"validating {n_extra} extras", flush=True)

    d = np.load(os.path.join(CACHE_DIR, "schemeM_train.npz"))
    ...
    print(f"PASS: {pass_n}/{n_extra} | WARN: {warn_n}/{n_extra} | FAIL: {fail_n}/{n_extra}", flush=True)
