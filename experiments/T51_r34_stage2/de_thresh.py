"""T51 DE 4D threshold search on T51 LOSO h_60 OOF predictions.

For a given tag (default 'loso_pred_h60'), loads
    {tag}_seed{S}_held{K}.parquet for S in seeds, K in 0..4
averages probs across seeds per fold (single seed by default), then runs:
...
def load_fold_avg(prefix: str, k: int, seeds: List[int]) -> pd.DataFrame:
...
def fold_arrays_from_dfs(folds):
    out = []
...
def main():
    ...
    ap.add_argument("--prefix", default="loso_pred_h60",
                    help="prediction file prefix; files are HERE/{prefix}_seed{S}_held{K}.parquet")
    ap.add_argument("--seeds", default="42")
    ap.add_argument("--out", default="thresh_results_seed42.json")
    args = ap.parse_args()

    seeds = [int(s) for s in args.seeds.split(",")]
    print(f"=== T51 DE thresh search prefix={args.prefix} seeds={seeds} ===", flush=True)
