"""T51 LOSO h=60 LightGBM training on schemeM cache (226 + 54 + 59 = 339 dim).

Same recipe as T44 but with extras = stage1 + stage2:
...
def main():
    ...
    ap.add_argument("--out-tag", default="r34_stage2")
    ...
    print(f"=== T51 LOSO h={H} variant={args.variant} seeds={seeds} syms={syms} ===", flush=True)
    ...
    n_extras = total_dim - base_dim  # 113
    extras_names_path = os.path.join(CACHE_DIR, "schemeM_extra_feat_names.txt")
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = f"T51-LOSO-h{H}-{args.variant}-{args.out_tag}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            wandb.init(
                project="liangwenbei",
                ...
                tags=["T51", "R34-stage2", "LOSO", f"h{H}"],
            )
    ...
    summary = {
        "task": "T51 R34 stage 2 features LOSO",
        "params": vars(args),
        "horizon": H,
