"""Predictor for T189: identical to T170 iter_018 v1 but NN inference uses torch (CUDA if available).

T170 baseline: iter_018 v1 (T127) — iter_015 v1 stack + per-sym beta abstain wrapper.
T189 change: _MLPNumpy replaced by _MLPTorch; torch CUDA used for NN forward if available.
All model weights, thresholds, conformal config unchanged — outputs match T170 within float32.
...
NN inference uses torch (CUDA if available, else CPU). LGB inference remains CPU.
...
class _MLPTorch:
    """MLP inference using torch (CUDA if available, else CPU).
    Weights loaded from npz and moved to target device at init.
    Numerically equivalent to _MLPNumpy within float32 precision.
    """
    ...
    def __init__(self, npz_path: str, device: str = "cpu"):
        ...
        self.feat_mean = torch.from_numpy(d["feat_mean"].astype(np.float32)).to(device)
        self.feat_std = torch.from_numpy(d["feat_std"].astype(np.float32)).to(device)
        self.keep_idx = d["keep_idx"].astype(np.int64)
        self.clip = float(d["clip"][0])

        self.W: List[torch.Tensor] = []
        self.b: List[torch.Tensor] = []
        self.LN_W: List[torch.Tensor] = []
        self.LN_b: List[torch.Tensor] = []
        for i in range(len(self.hidden)):
            self.W.append(torch.from_numpy(d[f"L{i}_W"].astype(np.float32)).to(device))
            self.b.append(torch.from_numpy(d[f"L{i}_b"].astype(np.float32)).to(device))
            if self.use_layernorm:
                self.LN_W.append(torch.from_numpy(d[f"LN{i}_W"].astype(np.float32)).to(device))
                self.LN_b.append(torch.from_numpy(d[f"LN{i}_b"].astype(np.float32)).to(device))
        self.WF = torch.from_numpy(d["LF_W"].astype(np.float32)).to(device)
        self.bF = torch.from_numpy(d["LF_b"].astype(np.float32)).to(device)
    ...
    def predict(self, X_in: np.ndarray) -> np.ndarray:
        ...
        for i in range(len(self.hidden)):
            h = F.linear(h, self.W[i], self.b[i])
            if self.use_layernorm:
                h = F.layer_norm(h, [h.shape[-1]], self.LN_W[i], self.LN_b[i], eps=1e-5)
            h = F.gelu(h, approximate="tanh")
        out = F.linear(h, self.WF, self.bF)
...
class Predictor:
    def __init__(self) -> None:
        ...
        # Load per-horizon LGB (CPU) and NN (torch, device) ensembles
        self._lgb_lists: Dict[int, List[lgb.Booster]] = {}
        self._nn_lists: Dict[int, List[_MLPTorch]] = {}
        ...
        for hcfg in self._horizons:
            ...
            if nn_paths:
                self._nn_lists[H] = [_MLPTorch(p, device=self._device) for p in nn_paths]
    ...
    @staticmethod
    def _ensemble_predict_nn(nns: List[_MLPTorch], X: np.ndarray) -> np.ndarray:
        if len(nns) == 1:
