"""T25: h_60 (and h_40) specialized 5-seed LOSO LightGBM trainer (GPU).

Background: iter_002 platform showed h_60 LOSO +6.30 -> platform +4.07 (gap only -2.23,
calibration good). h_40 LOSO +11.71 -> platform +2.02 (calibration medium). Short
horizons over-fit massively. So we specialize: 5-seed × 5-fold ensemble for h_60 and
h_40 with diverse hyperparam configs.

Per (seed, horizon, held_out_sym):
    train: (sym != held, dates 0..79)
    val:   (sym != held, dates 80..95)  -- early stopping
    test:  (sym == held, dates 96..119) -- held-out predictions

Diverse SEED_CONFIGS (more aggressive than T11) to maximize variance:
    42 : iter_002 baseline
    1  : low feature_fraction + low bagging
    7  : smaller trees + stronger reg
    13 : larger trees + weak reg
    100: extreme dropout + strong reg
...
Uses the 226-d schemeC cache from T5b and slices to 223-d (drops time-encoding tail)
to match iter_002/iter_004 feature space exactly.

GPU LightGBM forced (gpu_use_dp=False ~3x faster on RTX3090).
"""
...
# T25 *aggressive* diversity configs (more spread than T11).
SEED_CONFIGS = {
...
def main():
    ...
    ap.add_argument("--horizons", default="60",
                    help="comma-separated horizons (default 60)")
    ...
    ap.add_argument("--out-tag", default="T25")
    args = ap.parse_args()
    ...
    print(f"=== T25 LOSO Scheme C 223-d (GPU={args.use_gpu}), horizons={horizons}, seeds={seeds}, syms={target_syms} ===",
          flush=True)
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = f"T25-h60-specialized-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            init_kwargs = dict(
                project="liangwenbei",
                name=run_name,
                config={
                    ...
                    "horizons": horizons,
                    "seeds": seeds,
                    "diversity_strategy": "T25 aggressive (feature_fraction, bagging_fraction, num_leaves, lambda_l2)",
                    "seed_configs": {str(s): SEED_CONFIGS[s] for s in seeds},
                    **{k: v for k, v in vars(args).items() if k != "no_wandb"},
                },
                tags=["T25", "h60-specialized", "schemeC", "223d", "gpu"],
            )
            try:
    ...
    with open(summary_path, "w") as f:
        json.dump(sanitize_for_json({
            "task": "T25 multi-seed LOSO Scheme C 223-d",
    ...
    progress("loso_done", n_runs=len(all_results), elapsed_sec=time.time() - t0)
