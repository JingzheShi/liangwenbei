"""T25: Per-horizon threshold sweep on multi-seed ensemble OOF (h_60 / h_40).

Rule: side_max = max(p0, p2);
      take if side_max >= T and side_max > p1 + delta;
      pred 2 if p2 > p0 else 0; otherwise 1.

Operates on loso_pred_ensemble_h{H}_held{K}.parquet (output of aggregate.py).
Also evaluates raw argmax baselines (no threshold) and per-seed sweeps.
...
def main():
    ...
    out_results = {"per_horizon": {}, "horizons": horizons, "seeds": seeds,
                   ...
                   "task": "T25 per-horizon threshold sweep on multi-seed ensemble OOF"}
