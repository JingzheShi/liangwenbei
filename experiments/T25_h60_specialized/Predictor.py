"""Predictor for iter_005: h_60 specialized 5-seed ensemble + h_40 from iter_002.
...
Difference from iter_002 / iter_004:
  Per-horizon model file pattern is configurable in thresholds.json:
    - "model_files": ["model_h60_seed42.txt", ...] -> ensemble (avg probs)
    - "model_file":  "model_h40.txt"               -> single
  If neither is set we fall back to "model_h{H}.txt".

Bundle:
  thresholds.json            seeds list + per-horizon (T, delta, active, model_files)
  model_h{H}_seed{S}.txt     for ensemble horizons (h_60)
  model_h{H}.txt             for single-model horizons (h_40 reused from iter_002)
"""
    ...
    def __init__(self) -> None:
        ...
        for hcfg in self._horizons:
            if not hcfg.get("active", True):
                continue
            H = int(hcfg["h"])
            files = hcfg.get("model_files")
            if not files:
                single = hcfg.get("model_file") or f"model_h{H}.txt"
                files = [single]
            ms: List[lgb.Booster] = []
            for fn in files:
                mp = os.path.join(here, fn)
