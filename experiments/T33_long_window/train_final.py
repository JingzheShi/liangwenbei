"""T33: train final Scheme J 583-d models on (train + val) with test as early-stop.

Same pattern as T27 train_final_aug_a but on 583-d Scheme J cache. Each seed
trained with its T25-style cfg yields:
    final_model_h60_{variant}_seed{S}.txt

For iter_006 ensemble.
"""
...
# Same as T25/T27 SEED_CONFIGS (5 diverse hyperparam sets for ensembling)
SEED_CONFIGS = {
...
def main():
    ...
    print(
        f"=== T33 train_final Scheme J 583-d horizons={horizons} variant={args.variant} "
        f"seeds={seeds} use_gpu={args.use_gpu} ===",
        flush=True,
    )
    ...
    X_tr_orig = np.concatenate(
        [train_full["X"], val_full["X"]], axis=0
    ).astype(np.float32)
    ...
    assert len(feat_names) == X_tr_orig.shape[1], \
        f"mismatch {len(feat_names)} vs {X_tr_orig.shape[1]}"
    ...
    with open(summary_path, "w") as f:
        json.dump({
            "task": f"T33 train final Scheme J {args.variant}",
