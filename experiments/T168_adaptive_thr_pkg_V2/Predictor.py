"""Predictor V2_adaptive_thr: T168 per-batch percentile-anchored threshold variant.

V2: always ensure top 25% of rows by |pred| trade (floor_percentile=75).
Thresholds are raised to 75th pct of positives / 25th pct of negatives, but never
lowered below the static thr_up/dn. Conformal band applied on top.
...
class Predictor:
    ...
    def predict(self, batches: List[pd.DataFrame]) -> List[List[int]]:
        ...
        for hcfg in self._horizons:
            ...
            # V2: percentile-anchored — ensure at least top 25% of rows by |pred| trade
            FLOOR_PERCENTILE = 75  # top 25% always trades
            ...
            out_h = np.full(B, 1, dtype=np.int64)
            out_h[pred_dmid > (adaptive_thr_up + band_per_row)] = 2
            out_h[pred_dmid < -(adaptive_thr_dn + band_per_row)] = 0
            out[:, HORIZON_TO_IDX[H]] = out_h
        return out.tolist()
