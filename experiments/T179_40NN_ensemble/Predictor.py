"""T179: 40-NN ensemble (8 M7 variants × 5 seeds) for pure variance reduction.

Architecture identical to T170 (iter_018 v1 conformal wrapper + v2 LGB ensemble),
but NN ensemble expanded from 5 → 40 models:
  - v1 (T170 baseline, ep=11, lr=3e-5, lam=30): seeds 1,7,13,42,100
  - v2 (S1_ep7, ep=7):  seeds 1,7,13,42,100
  - v3 (S1_ep15, ep=15): seeds 1,7,13,42,100
  - v4 (S1_ep20, ep=20): seeds 1,7,13,42,100
  - v5 (S2_lr1e-5, lr=1e-5): seeds 1,7,13,42,100
  - v6 (S2_lr1e-4, lr=1e-4): seeds 1,7,13,42,100
  - v7 (S3_lam10, lam=10): seeds 1,7,13,42,100
  - v8 (S3_lam100, lam=100): seeds 1,7,13,42,100

All 40 NNs averaged equally -> pred_nn_combined.
Ensemble formula (unchanged from T170):
  pred = (1.0 * pred_nn + 1.5 * pred_lgb) / 2.5
Conformal abstain + per-sym beta (unchanged from T170/iter_018).
"""
...
# 8 variants x 5 seeds = 40 NN models
...
class Predictor:
    def __init__(self) -> None:
        ...
        # Conformal wrapper config (unchanged from T170)
        ...
        # Load per-horizon LGB ensemble (same as T170: 5 seeds from thresholds.json)
        ...
        # Load ALL 40 NN models: 8 variants x 5 seeds
        # Files: nn_h{H}_v{variant}_seed{seed}.npz
        self._nn_lists: Dict[int, List[_MLPNumpy]] = {}
        for hcfg in self._horizons:
            if not hcfg.get("active", True):
                continue
            H = int(hcfg["h"])
            nn_all: List[_MLPNumpy] = []
            for variant in NN_VARIANTS:
                for seed in NN_SEEDS:
                    np_path = os.path.join(here, f"nn_h{H}_{variant}_seed{seed}.npz")
                    if os.path.isfile(np_path):
                        nn_all.append(_MLPNumpy(np_path))
            if nn_all:
                self._nn_lists[H] = nn_all
    ...
    def _ensemble_predict_nn(nns: List[_MLPNumpy], X: np.ndarray) -> np.ndarray:
        """Average predictions from all 40 NNs (equal weight)."""
        if len(nns) == 1:
...
if __name__ == "__main__":
    ...
    df["sym"] = 1
    p = Predictor()
    n_nns = sum(len(v) for v in p._nn_lists.values())
    n_lgbs = sum(len(v) for v in p._lgb_lists.values())
    print(f"Loaded {n_nns} NNs, {n_lgbs} LGBs")
