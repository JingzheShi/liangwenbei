"""T7 threshold sweep on Scheme D1 LOSO OOF predictions.

Same (T, delta) grid as T5a. Decision rule:
...
def main():
    ...
    fold_dfs = {}
    for k in range(5):
        p = os.path.join(HERE, f"loso_pred_schemeD1_held{k}.parquet")
    ...
    for T in T_GRID:
        for d in D_GRID:
            ...
            print(f"  T={T:.2f} d={d:.2f} -> sum={sum_cum:+.4f} pos={n_pos}/5 active={sum_active:,}", flush=True)
    ...
    print(f"iter_001d (Scheme B) sum_cum_pnl = {ITER001D_SUM:+.4f}", flush=True)
    delta_vs_iter001d = best_sum - ITER001D_SUM
    print(f"\nbest D1 - iter_001d = {delta_vs_iter001d:+.4f}", flush=True)

    print(f"\n=== Per-fold @ best D1 (T={best_T}, delta={best_d}) ===", flush=True)
    best_pf_rows = []
    for k in range(5):
    ...
    SIGNIFICANCE = 0.5
    if delta_vs_iter001d >= SIGNIFICANCE:
        decision = "build_iter001f"
        decision_msg = (f"D1 best ({best_T}, {best_d}) beats iter_001d by {delta_vs_iter001d:+.4f} "
                        f">= {SIGNIFICANCE}; build iter_001f")
    else:
        decision = "no_iter001f"
        decision_msg = (f"D1 best ({best_T}, {best_d}) only beats iter_001d by {delta_vs_iter001d:+.4f} "
                        f"< {SIGNIFICANCE}; window zscore did not beat raw + threshold; keep iter_001d")
    print(f"\nDECISION: {decision_msg}", flush=True)

    out = {
        "task": "T7 window-zscore Scheme D1 LOSO + threshold sweep",
        "scheme": "D1",
        ...
        "delta_vs_iter001d": float(delta_vs_iter001d),
