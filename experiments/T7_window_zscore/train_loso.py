"""T7 LOSO training for Scheme D1 (raw 154 + window-zscore 154 = 308).
...
Sym-agnostic by construction (no sym ID in X). class_weight='balanced'.
...
def main():
    ...
    print(f"=== T7 LOSO scheme D1, target held-out syms = {target_syms} ===", flush=True)
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = f"T7-window-zscore-schemeD1-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            init_kwargs = dict(
                project="liangwenbei",
                name=run_name,
                config={
                ...
                tags=["T7-LOSO", "scheme-D1", "label_60", "window-zscore"],
            )
            try:
    ...
    for held_out_sym in target_syms:
        ...
        pred_path = os.path.join(HERE, f"loso_pred_schemeD1_held{held_out_sym}.parquet")
        te_df.to_parquet(pred_path, index=False)
        print(f"  predictions saved -> {pred_path}", flush=True)

        model_path = os.path.join(HERE, f"loso_model_schemeD1_held{held_out_sym}.txt")
    ...
    print(f"{'mean':>5s}  -  -  -  -  {cums_a.mean():>+10.4f}  {sing_a.mean():>+12.6f}  "
          f"{accs_a.mean():>9.4f}  {np.array(f05s).mean():>10.4f}", flush=True)
    print(f"{'sum':>5s}  -  -  -  -  {cums_a.sum():>+10.4f}", flush=True)
    ...
    out_path = os.path.join(HERE, "loso_results_schemeD1.json")
    ...
    if use_wandb:
        wandb.summary.update({
            "agg_D1/cum_pnl_mean": float(cums_a.mean()),
            "agg_D1/cum_pnl_std": float(cums_a.std(ddof=0)),
            "agg_D1/cum_pnl_sum": float(cums_a.sum()),
            "agg_D1/single_pnl_mean": float(sing_a.mean()),
            "agg_D1/accuracy_mean": float(accs_a.mean()),
            "agg_D1/n_positive_folds": int((cums_a > 0).sum()),
        })
        wandb.finish()
