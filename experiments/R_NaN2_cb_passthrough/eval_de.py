"""R-NaN2: DE asym thresh search per variant.

For each variant in {zero, min, max}:
...
def load_avg_pred(variant):
    paths = [os.path.join(OUT_DIR, f"pred_cb_huber_a0.001_{variant}_seed{s}.parquet")
             for s in SEEDS]
...
def main():
    print("=== R-NaN2 DE thresh search per variant ===", flush=True)
    out = {}
    ...
    # Pretty summary
    print("\n=== SUMMARY: DE LOSO 3-seed avg ===")
    print(f"{'variant':<12} {'k1_cum':>10} {'de_loso':>10} {'thr_up':>10} {'thr_dn':>10}  per_sym")
    for v in VARIANTS:
        if v not in out:
            continue
        d = out[v]
        per_str = "[" + ", ".join(f"{x:+.2f}" for x in d["de_per_sym"]) + "]"
        print(f"{v:<12} {d['k1_cum_pnl']:>+10.4f} {d['de_loso_cum_pnl']:>+10.4f} "
              f"{d['de_thr_up']:>10.6f} {d['de_thr_dn']:>10.6f}  {per_str}")
