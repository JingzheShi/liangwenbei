...
Combines:
1. T87 SPO+ DFL NN (nn_h60_seed*.npz) — 5 seeds
2. T97 multi-head NN (nn2_h60_seed*.npz) — 5 seeds
3. T95 GRU LOB sequence NN (nn3_h60_seed*.npz) — 5 seeds, window=100, top-5 LOB, 20 cols
...
Selective agreement filter (T158 adapted for 4-way):
  For syms in AGREEMENT_FILTER_SYMS = {0, 1, 2}:
    NN-side = weighted avg of T87 + T97 + T95: (w_nn*T87 + w_nn2*T97 + w_nn3*T95)/(w_nn+w_nn2+w_nn3)
    if sign(NN-side) != sign(pred_lgb): abstain (action=1, hold)
    else: use standard pipeline (conformal gate)
  For syms 3, 4: standard pipeline unchanged.
  For OOD syms: no agreement filter.
...
# T95 LOB input columns: top-5 levels, interleaved bid/bsize/ask/asize
...
# Selective agreement filter: only syms 0,1,2 (per T158/T163 analysis)
...
class _GRUNumpy:
    """T95 MiniGRU numpy inference: LayerNorm → GRU(1L) → fc → /target_scale.

    Input: (B, W=100, F=20) window-normalized LOB sequence.
    Gate order matches PyTorch: r (reset), z (update), n (new).
    """

    def __init__(self, npz_path: str):
        d = np.load(npz_path, allow_pickle=False)
        self.ln_w = d["ln_w"].astype(np.float64)   # (20,) LayerNorm weight
        self.ln_b = d["ln_b"].astype(np.float64)   # (20,) LayerNorm bias
        self.W_ih = d["W_ih"].astype(np.float64)   # (3H, F) [r,z,n] input weights
        self.W_hh = d["W_hh"].astype(np.float64)   # (3H, H) [r,z,n] hidden weights
        self.b_ih = d["b_ih"].astype(np.float64)   # (3H,)
        self.b_hh = d["b_hh"].astype(np.float64)   # (3H,)
        self.fc_W = d["fc_W"].astype(np.float64)   # (1, H)
        self.fc_b = d["fc_b"].astype(np.float64)   # (1,)
        self.target_scale = float(d["target_scale"][0])
        self.H = int(d["hidden_size"][0])

    def predict(self, x: np.ndarray) -> np.ndarray:
        """x: (B, W, 20) window-normalized float32. Returns (B,) predictions."""
        x = x.astype(np.float64)
        B, T, _ = x.shape
        H = self.H

        # LayerNorm(20) per timestep: normalize across 20 features
        mean = x.mean(axis=-1, keepdims=True)
        var = x.var(axis=-1, keepdims=True)
        x = (x - mean) / np.sqrt(var + 1e-5) * self.ln_w + self.ln_b
        ...
        for t in range(T):
            ...
            r = _sigmoid64(ih[:, :H] + hh[:, :H])
            z = _sigmoid64(ih[:, H:2*H] + hh[:, H:2*H])
            n = np.tanh(ih[:, 2*H:] + r * hh[:, 2*H:])
            h = (1.0 - z) * n + z * h
...
class Predictor:
    def __init__(self) -> None:
        ...
        # Pre-compute LOB column indices within raw_feat_cols
        self._lob_col_indices: List[int] = [
            self._raw_col_to_idx[c] for c in LOB_COLS_T95
            if c in self._raw_col_to_idx
        ]

        # Load per-horizon LGB, NN (T87), NN2 (T97), NN3 (T95 GRU) ensembles
    ...
    def _extract_lob_window(self, batches: List[pd.DataFrame]) -> np.ndarray:
        """Extract T95 LOB input: (B, W=100, F=20) window-normalized float32."""
        B = len(batches)
        X = np.empty((B, WINDOW, 20), dtype=np.float32)
        lob_idx = self._lob_col_indices
        for i, df in enumerate(batches):
            # Extract raw values and get the last WINDOW rows
            raw = df[self._raw_feat_cols].to_numpy(dtype=np.float32, copy=False)
            win = raw[-WINDOW:, :]               # (≤W, K)
            lob_win = win[:, lob_idx]             # (≤W, 20)
            # Pad if shorter than WINDOW
            if lob_win.shape[0] < WINDOW:
                pad = np.repeat(lob_win[:1], WINDOW - lob_win.shape[0], axis=0)
                lob_win = np.concatenate([pad, lob_win], axis=0)
            # Per-window per-feature normalization
            mean = lob_win.mean(axis=0, keepdims=True)
            std = lob_win.std(axis=0, keepdims=True) + 1e-8
            X[i] = np.clip((lob_win - mean) / std, -10.0, 10.0)
        return X

    def _extract_band_per_row(self, batches: List[pd.DataFrame]) -> np.ndarray:
        """Look up beta * sigma per row using sym from the last tick."""
        B = len(batches)
    ...
    def _ensemble_predict_gru(grus: List[_GRUNumpy], X_seq: np.ndarray) -> np.ndarray:
        """Run GRU ensemble on window-normalized LOB sequence (B, W, 20)."""
        if len(grus) == 1:
    ...
    def predict(self, batches: List[pd.DataFrame]) -> List[List[int]]:
        ...
        # Precompute T95 LOB window once (shared across horizons)
        ...
        for hcfg in self._horizons:
            ...
            if nn3s and w_nn3 > 0:
                if lob_window is None:
                    lob_window = self._extract_lob_window(batches)
                nn3_pred = self._ensemble_predict_gru(nn3s, lob_window)
                preds.append(nn3_pred)
                ws.append(w_nn3)
            if not preds:
                continue
            ...
            # Selective agreement filter (T158 adapted for 4-way ensemble):
            # NN-side = weighted avg of T87 + T97 + T95 (all NN components).
            # For syms 0,1,2: abstain if sign(NN-side) != sign(lgb_pred).
            if lgb_pred is not None and agree_filter_mask.any():
                nn_parts = []
                nn_ws = []
                if nn_pred is not None and w_nn > 0:
                    nn_parts.append(nn_pred); nn_ws.append(w_nn)
                if nn2_pred is not None and w_nn2 > 0:
                    nn_parts.append(nn2_pred); nn_ws.append(w_nn2)
                if nn3_pred is not None and w_nn3 > 0:
                ...
                if nn_parts:
                    ...
                    nn_combined = (nn_stacked * nn_ws_arr).sum(0) / nn_ws_arr.sum()
