"""T78: EV-gate threshold sweep on 5-seed-averaged regression-on-schemeQ predictions.

Identical pipeline to T75/ev_gate_eval.py but loads pred_T78_seed*.parquet.
Compares vs iter_013 LOSO-equiv +36.23 (the current SOTA) and iter_012 +26.44.
"""
...
SYMS = (0, 1, 2, 3, 4)
FEE = 0.0001
ITER013_LOSO = 36.2281
ITER012_LOSO = 26.44


def load_avg_pred(seeds):
    base = pd.read_parquet(os.path.join(HERE, f"pred_T78_seed{seeds[0]}.parquet"))
    p_sum = base["pred_dmid_norm"].to_numpy(np.float64).copy()
    n = len(base)
    for s in seeds[1:]:
        df_s = pd.read_parquet(os.path.join(HERE, f"pred_T78_seed{s}.parquet"))
        if len(df_s) != n:
...
def main():
    ...
    print(f"=== T78 EV-gate sweep seeds={seeds} ===", flush=True)
    ...
    print(f"      iter_012 ref: {ITER012_LOSO:+.4f}    iter_013 ref: {ITER013_LOSO:+.4f}\n", flush=True)
    sym_sweep = []
    ...
    print(f"vs iter_013 +{ITER013_LOSO:.4f}  (iter_012 +{ITER012_LOSO:.2f}):", flush=True)
    print(f"  best_sym_sweep      vs iter_013: {best_sweep['sum_per_sym']-ITER013_LOSO:+.4f}", flush=True)
    print(f"  de_single_total     vs iter_013: {de_single_total-ITER013_LOSO:+.4f}", flush=True)
    print(f"  de_loso_sum         vs iter_013: {de_loso_sum-ITER013_LOSO:+.4f}", flush=True)

    out = {
        "task": "T78 EV-gate sweep on 5-seed regression-on-schemeQ avg predictions",
        "experiment": "T78_regression_ev_time",
        "seeds": seeds,
        "n_test": len(df),
        "fee_rate": FEE,
        "iter_013_loso_ref": ITER013_LOSO,
        ...
        "compare_iter013": {
            ...
            "best_sweep_vs_iter013": best_sweep["sum_per_sym"] - ITER013_LOSO,
            "de_single_total_vs_iter013": de_single_total - ITER013_LOSO,
            "de_loso_sum_vs_iter013": de_loso_sum - ITER013_LOSO,
        },
    }
