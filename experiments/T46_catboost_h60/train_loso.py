"""T46: CatBoost LOSO Scheme C 223-d, h_60 only, 5 seeds × 5 folds, aug_a [0.80, 1.20].
...
def main():
    ...
    print(
        f"=== T46 CatBoost h_{H} LOSO: seeds={seeds} syms={target_syms} "
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = (
                f"T46-cb-h60-{args.tag}-seeds{','.join(str(s) for s in seeds)}-"
            ...
            init_kwargs = dict(
                ...
                tags=["T46", "catboost", "schemeC", "223d", f"h{H}", args.tag],
            )
            try:
    ...
    for s in seeds:
        ...
        for held in target_syms:
            ...
            interim = {
                "task": f"T46 CatBoost h_{H} LOSO ({args.boosting_type}, tag={args.tag})",
                "horizon": H,
                "seeds": seeds,
                "syms": target_syms,
