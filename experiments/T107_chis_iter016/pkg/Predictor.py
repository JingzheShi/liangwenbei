"""Predictor for iter_016 CHIS (Cross-Horizon Independent Submission).

ALL 5 horizons {5, 10, 20, 40, 60} are independently activated with their own
optimal ensemble + asymmetric EV gate. The platform scores via MAX over
horizons, so this is a 0-downside extension of iter_015 v2's h=60-only stack.

Per-horizon stacks:

  h=5/10/20/40 ("lgb_cb_2way"):
    pred_h = (w_lgb * mean_lgb_h + w_cb * mean_cb_h) / (w_lgb + w_cb)
    where mean_*_h is averaged across 5 T98 seeded models (LGB regression /
    CatBoost regression) trained per-horizon on the schemeP 359-d feature cache.

  h=60 ("fourway_iter015v2"):
    pred_h = (1.0 * mean_T87_NN + 1.5 * mean_T89_CB
              + 0.7 * mean_T99_huber + 0.7 * mean_T95_GRU) / 3.9
    Identical to experiments/T106_iter015v2_pkg/Predictor.py (4-way iter_015 v2).
...
Predictor.predict(batches) returns a (B, 5) Python list-of-lists, where column
index i corresponds to horizon HORIZON_LIST[i] = (5, 10, 20, 40, 60).
...
class _MLPNumpy:
    ...
    Pipeline: standardize -> [Linear -> LayerNorm -> GELU]xN -> Linear -> /target_scale.
    """
...
class _GRUNumpy:
    ...
    Pipeline: raw window (B, W, 20) -> per-window standardize -> clip(+-clip)
              -> LayerNorm -> GRU (hidden=64) -> last-step -> Linear -> /target_scale.
    """
...
class Predictor:
    def __init__(self) -> None:
        ...
        # h=5/10/20/40 -> {"lgb": [boosters], "cb": [models], "w_lgb", "w_cb",
        #                  "thr_up", "thr_dn"}.
        # h=60 -> {"nn": [...], "cb": [...], "huber": [...], "gru": [...],
        #          "w_nn"/"w_cb"/"w_huber"/"w_gru", "thr_up", "thr_dn"}.
        ...
        for hcfg in self._horizons:
            ...
            if kind == "lgb_cb_2way":
                ...
                for s in seeds:
                    ...
                    cb_p = os.path.join(here, f"model_T89_seed{s}.cbm")
                    ...
                    if os.path.isfile(gr_p):
                        gru_pool.append(_GRUNumpy(gr_p))
                self._h_long = {
                    "horizon": H,
                    "nn": nn_pool,
                    "cb": cb_pool,
                    ...
                    "w_t89": float(hcfg.get("w_t89", 1.5)),
                    "w_huber": float(hcfg.get("w_huber", 0.7)),
                    "w_gru": float(hcfg.get("w_gru", 0.7)),
                    "thr_up": float(hcfg.get("thr_up", 1.0)),
                    "thr_dn": float(hcfg.get("thr_dn", 1.0)),
                }
            else:
    ...
    def predict(self, batches: List[pd.DataFrame]) -> List[List[int]]:
        ...
        if self._h_long is not None:
            ...
            if mp["gru"] and mp["w_gru"] > 0:
                preds_w.append(
                    (self._ensemble_avg([g.predict(x_gru) for g in mp["gru"]]),
                     mp["w_gru"])
                )
            if preds_w:
                wsum = sum(w for _, w in preds_w)
