"""Predictor for iter_019 v2W (T158): v2 stack + selective agreement filter.

Builds on iter_019 v2 (T87 SPO+ NN + T75 L2-retrained LGB + iter_018 conformal):

  pred_lgb = mean over 5 LightGBM regression boosters (T75 full-retrain)
  pred_nn  = mean over 5 small MLPs (T87 SPO+ DFL, byte-identical to iter_015 v1)
  pred     = (w_nn * pred_nn + w_lgb * pred_lgb) / (w_nn + w_lgb), w_nn=1.0, w_lgb=0.5

iter_018 conformal wrapper:
  effective_thr_up = thr_up + beta_sym * sigma_sym
  effective_thr_dn = thr_dn + beta_sym * sigma_sym

NEW in v2W (T158): Selective per-sym agreement filter.
  Motivation: per-sym analysis showed NN/LGB agreement strongly predicts trade quality
  for syms 0-2 but not for syms 3-4.
  For syms in AGREEMENT_FILTER_SYMS = {0, 1, 2}:
    if sign(pred_nn) != sign(pred_lgb): abstain (action=1, hold)
    else: use standard pipeline (conformal gate)
  For syms 3, 4: standard pipeline unchanged (agreement filter hurts these syms).
  For OOD syms: no agreement filter (fallback to standard).

  T158 holdout eval (dates 96-119):
    v2 baseline:       42.165 (173729 trades)
    selective W1:      43.400 (170036 trades, delta=+1.235)
...
# T158 selective agreement filter: only apply to these syms (per holdout analysis).
# Syms 0,1,2 benefit (+0.53, +0.53, +0.18 delta); syms 3,4 do NOT (-0.15, -0.73).
...
class Predictor:
    ...
    def predict(self, batches: List[pd.DataFrame]) -> List[List[int]]:
        ...
        # Rows where agreement filter applies (syms 0,1,2)
        ...
        for hcfg in self._horizons:
            ...
            # Selective agreement filter: abstain if NN and LGB disagree on sign
            # (applied only to syms in AGREEMENT_FILTER_SYMS)
            if lgb_pred is not None and nn_pred is not None and agree_filter_mask.any():
