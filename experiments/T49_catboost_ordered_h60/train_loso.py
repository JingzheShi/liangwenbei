"""T49: CatBoost Ordered Boosting LOSO Scheme C 223-d, h_60 only, 5 seeds × 5 folds, aug_a [0.80, 1.20].

Implements R35 idea #2 — CatBoost Ordered Boosting on h_60 with the same
LOSO split / aug_a regime as iter_005b (LGB).
...
Outputs (per (seed, fold)):
    pred_seed{S}_held{K}.parquet   -- full schema for DE thresh
    model_seed{S}_held{K}.cbm      -- CatBoost native model file (CPU/GPU loadable)
...
Compliance with CRITICAL_CONSTRAINTS:
  ...
  * sym-agnostic model (CatBoost on raw 223-d feature columns)
"""
...
def train_one_fold(seed, held, X_tr_full, y_tr_full, sym_tr,
    ...
    # CatBoost constraint: GPU does NOT support Ordered+MultiClass.
    # Force CPU when boosting=Ordered and loss=MultiClass.
    use_gpu_effective = args.use_gpu and (boosting_type != "Ordered")
    if use_gpu_effective:
        model_kwargs["task_type"] = "GPU"
        model_kwargs["devices"] = args.devices
    else:
        if args.use_gpu and boosting_type == "Ordered":
            print("    [T49] Ordered+MultiClass not supported on GPU; using CPU.", flush=True)
...
def main():
    ...
    ap.add_argument("--boosting-type", default="Ordered",
                    choices=("Ordered", "Plain"),
                    help="CatBoost boosting type. Ordered is the headline experiment.")
    ap.add_argument("--tag", default="ordered",
                    help="Output filename tag, e.g. 'ordered' or 'plain' for ablation.")
    ...
    print(
        f"=== T49 CatBoost h_{H} LOSO: seeds={seeds} syms={target_syms} "
    ...
    # 226 -> 223 (drop trailing time-encoding cols)
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = (
                f"T49-cb-h60-{args.tag}-seeds{','.join(str(s) for s in seeds)}-"
            ...
            init_kwargs = dict(
                ...
                tags=["T49", "catboost", "schemeC", "223d", f"h{H}", args.tag, "ordered"],
            )
            try:
    ...
    for s in seeds:
        ...
        for held in target_syms:
            ...
            # incremental save in case we crash
            interim = {
                "task": f"T49 CatBoost h_{H} LOSO ({args.boosting_type}, tag={args.tag})",
                "horizon": H,
                "seeds": seeds,
                "syms": target_syms,
