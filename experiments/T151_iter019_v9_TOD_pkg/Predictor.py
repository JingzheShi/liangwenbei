"""Predictor for iter_019 v9 (T151): v2 + 5 Time-Of-Day features appended to LGB input.

iter_019 v9 = iter_019 v2 (T87 NN + full-retrain LGB 5 seeds + iter_018 conformal)
 + 5 TOD features appended to schemeP 359 → 364 for LGB only.

  pred_lgb = mean over 5 LightGBM regression boosters (364-d input, full-retrain + TOD)
...
TOD features (appended last, positions 359-363):
...
Trading session: AM 9:40-11:20 (100 min, sess_idx=0), PM 13:10-14:50 (100 min, sess_idx=1).
"time" column must be in config.json feature list so the platform passes it.
...
class Predictor:
    def __init__(self) -> None:
        ...
        self._ffb = _load_module(here, "fast_features_batch.py", "iter_019v9_ffb")
        ...
        # Load per-horizon LGB (364-d) and NN (359-d) ensembles
    ...
    def _compute_batch_features(self, batches: List[pd.DataFrame]) -> Tuple[np.ndarray, np.ndarray]:
        """Compute features for LGB (364-d) and NN (359-d).

        Returns:
          feats_lgb: (N, 364) — 154 raw + 205 extras + 5 TOD
        ...
        feats_364 = np.concatenate([feats_359, tod], axis=1)  # (N, 364)

        return feats_364, feats_359
...
if __name__ == "__main__":
    ...
    df["sym"] = 1
    # Simulate time column: AM session (9:40 + 99 * 3s)
    import datetime as _dt
    times = [_dt.time(9, 40, i * 3 % 60, 0) for i in range(100)]
