...
Loads pred_T81_seed{S}.parquet for S in 1,7,13,42,100, averages predicted
...
def load_avg_pred(seeds):
    ...
    n = len(base)
    for s in seeds[1:]:
        df_s = pd.read_parquet(os.path.join(HERE, f"pred_T81_seed{s}.parquet"))
        if len(df_s) != n:
...
def main():
    ...
    out = {
        "task": "T81 EV-gate sweep on 5-seed NN regression Δmid avg predictions",
        "experiment": "T81_nn_regression_pnl",
        "seeds": seeds,
        "n_test": len(df),
        "fee_rate": FEE,
