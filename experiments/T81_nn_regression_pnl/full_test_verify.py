"""Verify iter_014 ensemble cum_pnl on the full 442k test using cached features.
...
applies the EV gate from thresholds.json, and computes per-sym cum_pnl.

Expected: matches +38.28 LOSO-equiv from ev_gate_ensemble_results.json.
"""
...
from numpy_nn_inference import MLPNumpy
...
def main():
    print("=== iter_014 full-test verification ===", flush=True)
    ...
    # Build the same keep_idx as Predictor would (370 → 359)
    ...
    nn_models = []
    for s in SEEDS:
        p = os.path.join(HERE, f"model_T81_seed{s}.npz")
        nn_models.append(MLPNumpy(p))
    print(f"  loaded {len(nn_models)} NN in {time.time()-t0:.1f}s", flush=True)

    # LGB inference (chunked)
    t0 = time.time()
    ...
    # NN inference (chunked)
    t0 = time.time()
    p_nn_acc = np.zeros(len(X), dtype=np.float64)
    for nn in nn_models:
        for s in range(0, len(X), BATCH):
            # MLPNumpy.standardize takes 359-d X already
    ...
    print(f"  NN ensemble inference in {time.time()-t0:.1f}s", flush=True)

    # Combine: w_nn=1.0, w_lgb=1.5
    w_nn, w_lgb = 1.0, 1.5
    ...
    print(f"  (offline ev_gate_ensemble target: +38.2810)", flush=True)
    print(f"  diff: {sum_per_sym - 38.2810:+.6f}", flush=True)
