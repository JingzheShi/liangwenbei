"""End-to-end check for iter_014 Predictor:
  1. Load a few real test parquets
  2. Build batches (100-tick windows) like the platform would
  3. Call Predictor.predict
  4. Compare ensemble-avg (5 LGB + 5 NN, weighted) to offline pred
  5. Compute LOSO-equiv cum_pnl on the predicted actions
  6. Sanity: shuffle batch order — predictions must be invariant per-row
  7. Sanity: inject sym=99 in input — must not crash
"""
...
sys.path.insert(0, os.path.join(HERE, "iter014_pkg"))
...
def load_test_batches(n_files: int = 6, n_test_per_file: int = 8):
    """Load a handful of parquets from test dates 96-119; build random 100-tick batches."""
    test_files = []
    ...
    batches = []
    truth = []  # (sym, date, t, mp_t, mp_t60, label_60)
    for fn in test_files:
        df = pd.read_parquet(fn)
        # We need t in [99, 2001-1-60] = [99, 1940] for valid 60-tick lookahead
        t_max = len(df) - 60 - 1
...
def main():
    print("=== iter_014 end-to-end check ===", flush=True)
    ...
    # Sanity: shuffle then predict, expect same result per row
    ...
    if "sym" in batches[0].columns:
        ...
        # Predictions should be the same as for original (sym is not used)
        diff = int((out99_arr != out_arr[:8]).sum())
        # Note: depending on the feature pipeline, sym might or might not enter
        # via `time` column or implicit feature. We just check no crash.
        print(f"  ✅ sym=99 injection: no crash, predictions differ in {diff}/{out99_arr.size} cells",
              flush=True)

    # Compare to offline ensemble pred (load 5-seed avg from ev_gate_ensemble eval; for spot check)
    # We can sanity-check the magnitude of pred_dmid for a few batches
    feats = pred_obj._compute_batch_features(batches[:8])
    lgbs = pred_obj._lgb_lists[60]
    nns = pred_obj._nn_lists[60]
    p_lgb = pred_obj._ensemble_predict_lgb(lgbs, feats)
    p_nn = pred_obj._ensemble_predict_nn(nns, feats)
    w_nn, w_lgb = pred_obj._weights[60]
    p_combined = (w_nn * p_nn + w_lgb * p_lgb) / (w_nn + w_lgb)
    print(f"  spot-check first 8 batches:", flush=True)
    for i in range(min(5, len(batches))):
        print(f"    [{i}] sym={truth[i]['sym']} t={truth[i]['t']}  "
              ...
              f"action={out_arr[i, 4]}  Δmid_actual={truth[i]['mp_th']-truth[i]['mp_t']:+.4f}",
              flush=True)
