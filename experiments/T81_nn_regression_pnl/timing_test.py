"""Timing test for iter_014 Predictor: 1024-batch inference time.

Replicates the sanity_and_timing.py from T75 setup but for the new
NN+LGB ensemble Predictor.
"""
...
def build_batches(n: int = 1024):
    """Build n random 100-tick windows from a single test parquet."""
...
def main():
    print("=== iter_014 timing test ===", flush=True)
    ...
    print(f"  iter_013 reference: ~0.63s (LGB-only)", flush=True)
