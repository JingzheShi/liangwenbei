"""T70 DE 4D threshold search on 5-seed-averaged predictions.

Loads pred_T70_seed{S}.parquet for S in 1,7,13,42,100, averages probs across
seeds (per row), then computes:
  ...
  3) DE 4D treating sym0..4 as 5 folds (objective = SUM of per-sym cum_pnl)
     => LOSO-equivalent comparison vs iter_011 +25.94

Saves de_results_t70.json.
"""
...
def load_avg_pred(seeds):
    base = pd.read_parquet(os.path.join(HERE, f"pred_T70_seed{seeds[0]}.parquet"))
    p_sum = base[PROB_COLS].to_numpy(np.float64).copy()
    n = len(base)
    for s in seeds[1:]:
        df_s = pd.read_parquet(os.path.join(HERE, f"pred_T70_seed{s}.parquet"))
        if len(df_s) != n:
...
def main():
    ...
    print(f"=== T70 DE 4D thresh seeds={seeds} ===", flush=True)
    ...
    out = {
        "task": "T70 DE 4D thresh (V4 walk-fwd + Stage 5 features)",
        "experiment": "T70_v4_stage5",
        "seeds": seeds,
        "n_test": len(df),
        "raw_argmax": {
        ...
        "compare_iter011": {
            ...
            "t70_de_single_total_vs_iter011": de_single_total - ITER011_LOSO,
            "t70_de_loso_sum_vs_iter011": de_loso_sum - ITER011_LOSO,
        },
    }
    out_path = os.path.join(HERE, "de_results_t70.json")
    ...
    print(f"vs iter_011 +{ITER011_LOSO:.2f}: t70_de_loso_diff={de_loso_sum - ITER011_LOSO:+.4f}", flush=True)
