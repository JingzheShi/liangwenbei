"""T70: V4 walk-forward val + Stage 5 features (schemeP cache).

Uses T68 schemeP cache (370 dims = 154 raw + 196 baseline extras + 20 stage5).
Drop set:
...
# Drop set: T59 fail names + Stage 5 fail
T59_FAIL_NAMES = [
...
BASE_RAW_DIM = 154  # raw OHLC + LOB cols (no time, no sym, no date)
...
def build_v4_split(train_full, slicer):
    """V4 walk-forward: train=date 0-75 (95%), val=date 76-79 (5%) inside train cache."""
...
def main():
    ...
    ap.add_argument("--out-tag", default="v4_stage5")
    ap.add_argument("--no-wandb", action="store_true")
    args = ap.parse_args()

    H = args.horizon
    seed = args.seed
    print(f"=== T70 V4 + Stage 5 seed={seed} h={H} ===", flush=True)
    ...
    # Build V4 split (train=0-75, val=76-79 inside train_full)
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = (f"T70-V4-stage5-seed{seed}-{args.out_tag}-"
    ...
    # Save model
    model_path = os.path.join(HERE, f"model_T70_seed{seed}.txt")
    ...
    pred_path = os.path.join(HERE, f"pred_T70_seed{seed}.parquet")
    te_df.to_parquet(pred_path, index=False)
    print(f"  saved {pred_path} ({len(te_df):,} rows)", flush=True)

    summary = {
        "task": f"T70 V4 stage5 seed={seed}",
        "seed": seed,
        "horizon": H,
        "split_info": info,
        "n_features": feat_dim,
    ...
    out_path = os.path.join(HERE, f"summary_T70_seed{seed}.json")
