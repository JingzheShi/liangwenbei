"""Verify iter_017 v1 ensemble cum_pnl on the full 442k test using cached features.

Loads schemeP_test.npz cached features (442k × 370) + HYD cache + 5 NN + 5 LGB.

Replicates Predictor logic:
  - 359 features for NN (= 370 minus 11 fail names)
  - 375 features for LGB = 359 + 16 HYD
  - w_nn=1.0, w_lgb=1.5
  - thr_up=0.000421, thr_dn=0.000186 (iter_015 v1 conservative)
"""
...
def main():
    print("=== iter_017 v1 full-test verification (T87 SPO+ DFL + T123 LGB) ===", flush=True)
    ...
    X_kept = X[:, keep_idx]
    print(f"  X_kept (NN input): {X_kept.shape}", flush=True)

    hyd_test = np.load(os.path.join(HYD_CACHE, "hyd_test.npy"))
    X_lgb = np.concatenate([X_kept, hyd_test], axis=1).astype(np.float32)
    print(f"  X_lgb (LGB input): {X_lgb.shape}", flush=True)

    t0 = time.time()
    lgb_models = []
    for s in SEEDS:
        p = os.path.join(HERE, f"model_T123_seed{s}.txt")
    ...
    for m in lgb_models:
        for s in range(0, len(X), BATCH):
            p_lgb_acc[s:s+BATCH] += m.predict(X_lgb[s:s+BATCH])
    ...
    thr_up, thr_dn = 0.000421, 0.000186
    ...
    print(f"  vs iter_015 v1 (+40.13 DE-LOSO opt / +39.08 same conservative thr): "
          f"{sum_per_sym - 40.13:+.4f} vs DE / {sum_per_sym - 39.08:+.4f} vs conservative",
          flush=True)
    ...
    out = {
        ...
        "vs_iter015_v1_de_opt_4013": sum_per_sym - 40.13,
        "vs_iter015_v1_same_thr_3908": sum_per_sym - 39.08,
        "vs_iter014": sum_per_sym - 38.281,
    }
