"""T123: T75 LGB L2 + HYD features + monotone constraints + date-decay sample weight.

Stack on iter_017:
  - HYD interaction features (16 dims, R3 cache)
  - Monotone constraints (T117 list, sign per feature)
  - Date-decay sample weight: sw = sw_class_balanced * (1.0 + 1.0 * date / 79)

5-seed (1, 7, 13, 42, 100) — full ensemble for iter_017 final submission.

Output: model_T123_seed{S}.txt + pred_T123_seed{S}.parquet.
"""
...
# 5-seed config (same as T75 baseline)
SEED_CONFIGS = {
...
def main():
    ...
    seed = args.seed
    print(f"=== T123 LGB L2 + HYD + monotone + date-decay seed={seed} ===", flush=True)
    progress("loading", seed=seed)
    ...
    leak = forbidden & set(feat_names_base)
    ...
    assert hyd_train.shape[0] == train_full["X"].shape[0], (hyd_train.shape, train_full["X"].shape)
    assert hyd_test.shape[0] == test_full["X"].shape[0]
    print(f"  +HYD: {hyd_train.shape[1]} dims", flush=True)

    feat_names_full = feat_names_base + hyd_names
    ...
    X_tr = np.concatenate([Xb_tr, hyd_train[m_t]], axis=1)
    X_va = np.concatenate([Xb_va, hyd_train[m_va]], axis=1)
    del Xb_tr, Xb_va
    ...
    date_tr_v = train_full["date"][m_t].astype(np.float32)  # for date-decay sample weight
    ...
    X_te = np.concatenate([Xb_te, hyd_test], axis=1)
    del Xb_te
    ...
    for fn, sign in MONOTONE_FEATS.items():
        if fn in feat_names_full:
            ...
            mc[idx] = sign
            n_constrained += 1
    print(f"  monotone-constrained features: {n_constrained}/{len(feat_names_full)}", flush=True)
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = f"T123-iter017v1-seed{seed}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            wandb.init(project="liangwenbei", entity="cjxh21-Tsinghua University",
                       name=run_name,
                       config={"seed": seed,
                               "use_hyd": True, "use_monotone": True,
                               "use_date_decay": True,
                               "n_features": X_tr.shape[1],
                               "objective": "regression_l2",
                               "n_constrained": n_constrained,
                               "horizon": H,
                               "task": "T123_iter017_v1_pkg"},
                       tags=["T123", "iter017", "lgb_l2", "hyd", "monotone", "date_decay", f"seed{seed}"])
    ...
    progress("training", seed=seed)

    # aug_a per-(sample, feat) scale [0.8, 1.2], ratio=1.0
    ...
    date_tr_full = np.concatenate([date_tr_v, date_tr_v], axis=0)
    del X_tr, X_aug

    # Date-decay sample weight: sw = sw_class_balanced * (1.0 + 1.0 * date/79)
    sw_cb_tr = class_balanced_weight(y_cls_tr_full, num_class=NUM_CLASS)
    date_decay = (1.0 + 1.0 * date_tr_full / 79.0).astype(np.float32)
    sw_tr = sw_cb_tr * date_decay
    print(f"  sw stats: cb mean={sw_cb_tr.mean():.4f}  decay range=[{date_decay.min():.3f}, {date_decay.max():.3f}]"
          f"  combined mean={sw_tr.mean():.4f}", flush=True)
    ...
    model_path = os.path.join(HERE, f"model_T123_seed{seed}.txt")
    ...
    pred_path = os.path.join(HERE, f"pred_T123_seed{seed}.parquet")
    ...
    summary = {
        "task": f"T123 iter017_v1 LGB L2 (HYD+monotone+date-decay) seed={seed}",
        "seed": seed, "horizon": H,
        "objective": "regression_l2",
        "use_hyd": True, "use_monotone": True, "use_date_decay": True,
        "date_decay_formula": "sw = sw_class_balanced * (1.0 + 1.0 * date / 79)",
        "n_constrained": n_constrained,
    ...
    out_path = os.path.join(HERE, f"summary_T123_seed{seed}.json")
