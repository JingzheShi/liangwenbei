"""T22 threshold sweep — single seed (42), per scheme.

Reads loso_pred_{scheme}_h{H}_held{k}.parquet and runs the same (T, delta) grid
as T21b. Outputs sweep_results_{scheme}_h{H}.csv and threshold_results_{scheme}.json.
"""
...
def main():
    ...
    ap.add_argument("--scheme", required=True, choices=("Ia", "Ib", "Ic"))
    ...
    fold_dfs = {}
    for k in SYMS:
        p = os.path.join(HERE, f"loso_pred_{args.scheme}_h{H}_held{k}.parquet")
        if not os.path.exists(p):
            sys.exit(f"missing {p}")
        fold_dfs[k] = pd.read_parquet(p)

    n_total = sum(len(v) for v in fold_dfs.values())
    print(f"=== T22 sweep scheme={args.scheme} h={H} ({n_total:,} OOF rows) ===", flush=True)
    ...
    out_csv = os.path.join(HERE, f"sweep_results_{args.scheme}_h{H}.csv")
    ...
    iter_002_baseline = 21.86
    iter_003_baseline = 22.22
    decision = "build_iter_004" if best["sum_cum_pnl"] > 22.5 else "no_iter_004"
    print(f"\n=== DECISION ===")
    print(f"iter_002 baseline       : +{iter_002_baseline:.2f}")
    print(f"iter_003 baseline       : +{iter_003_baseline:.2f}")
    print(f"T22 {args.scheme} best   : {best['sum_cum_pnl']:+.4f}")
    print(f"diff vs iter_002        : {best['sum_cum_pnl'] - iter_002_baseline:+.4f}")
    print(f"diff vs iter_003        : {best['sum_cum_pnl'] - iter_003_baseline:+.4f}")
    print(f"decision                : {decision} (need > +22.5)")

    out = {
        "task": f"T22 {args.scheme} threshold sweep h={H}",
        "scheme": args.scheme, "horizon": H,
        "T_grid": T_GRID, "delta_grid": D_GRID,
        "baseline_argmax": {"sum_cum_pnl": float(base_sum), "per_fold": base_pf},
        "best": {
        ...
        "iter_003_baseline": iter_003_baseline,
        "diff_vs_iter_002": float(best["sum_cum_pnl"]) - iter_002_baseline,
        "diff_vs_iter_003": float(best["sum_cum_pnl"]) - iter_003_baseline,
        "elapsed_sec": time.time() - t0,
    }
    out_json = os.path.join(HERE, f"threshold_results_{args.scheme}.json")
