"""LOSO multi-horizon LightGBM training on Scheme F (380-d) cache.
...
def main():
    ...
    ap.add_argument("--out-tag", default="schemeF")
    args = ap.parse_args()

    horizons = [int(x) for x in args.horizons.split(",")]
    target_syms = [int(x) for x in args.syms.split(",")]
    print(f"=== T10 LOSO Scheme F (380-d), horizons={horizons}, syms={target_syms} ===", flush=True)
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = f"T10-schemeF-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            init_kwargs = dict(
                project="liangwenbei",
                name=run_name,
                config={
                    ...
                    "components": "raw 154 + zscore100 154 + T3 72",
                    **{k: v for k, v in vars(args).items() if k != "no_wandb"},
                },
                tags=["T10-LOSO", "schemeF", "multihorizon", "z-score+T3"],
            )
            try:
