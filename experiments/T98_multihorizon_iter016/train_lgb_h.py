"""T98: LGB regression for arbitrary horizon h ∈ {5, 10, 20, 40, 60}.

Fork of T75 train_regr.py — uses the schemeP cache's per-horizon labels and
mp_t{H} columns. V4 split (train date 0-75, val 76-79). 5 seeds (matches T75).

Saves model_lgb_h{H}_seed{S}.txt and pred_lgb_h{H}_seed{S}.parquet.
"""
...
def main():
    ...
    H = args.horizon
    seed = args.seed
    print(f"=== T98 LGB regression Δmid h={H} seed={seed} obj={args.objective} ===", flush=True)
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = (f"T98-lgb-h{H}-seed{seed}-{args.objective}-"
            ...
            wandb.init(
                ...
                tags=["T98", "lgb", "regression", "multihorizon", f"h{H}", args.objective],
            )
    ...
    model_path = os.path.join(HERE, f"model_lgb_h{H}_seed{seed}.txt")
    ...
    pred_path = os.path.join(HERE, f"pred_lgb_h{H}_seed{seed}.parquet")
    ...
    summary = {
        "task": f"T98 LGB regression Δmid h={H} seed={seed}",
        "seed": seed,
        "horizon": H,
        "objective": args.objective,
        "split_info": info,
    ...
    out_path = os.path.join(HERE, f"summary_lgb_h{H}_seed{seed}.json")
