"""Predictor for iter_016: multi-horizon LGB + CB regression ensemble + EV gate.

Each of 5 horizons h ∈ {5, 10, 20, 40, 60} has:
  - up to 5 LightGBM regression boosters (T98 / T75)
  - up to 5 CatBoost regression models  (T98 / T89)
  - per-horizon weights (w_lgb, w_cb) and asymmetric thresholds (thr_up, thr_dn)

  pred_h = (w_lgb * mean_lgb(features) + w_cb * mean_cb(features)) / (w_lgb + w_cb)
  action_h = 2 if pred_h > thr_up else (0 if pred_h < -thr_dn else 1)
...
Compliance with platform contract (CRITICAL_CONSTRAINTS.md):
  ...
  - sym-agnostic; works on any sym ID 0..N
...
class Predictor:
    def __init__(self) -> None:
        ...
        self._ffb = _load_module(here, "fast_features_batch.py", "iter_016_ffb")
        ...
        for hcfg in self._horizons:
            ...
            if cb_paths:
                if self._CatBoostRegressor is None:
                    from catboost import CatBoostRegressor
                    self._CatBoostRegressor = CatBoostRegressor
                cb_models = []
                for p in cb_paths:
                    m = self._CatBoostRegressor()
                    m.load_model(p)
                    cb_models.append(m)
                self._cb_lists[H] = cb_models
            self._weights[H] = (float(hcfg.get("w_lgb", 1.0)),
                                float(hcfg.get("w_cb", 1.0)))
    ...
    @staticmethod
    def _ensemble_predict_cb(models: list, X: np.ndarray) -> np.ndarray:
        if len(models) == 1:
            return models[0].predict(X).astype(np.float32, copy=False)
        acc = None
        for m in models:
    ...
    def predict(self, batches: List[pd.DataFrame]) -> List[List[int]]:
        ...
        for hcfg in self._horizons:
            ...
            w_lgb, w_cb = self._weights.get(H, (1.0, 1.0))
            ...
            if lgbs and w_lgb > 0:
                ...
                ws.append(w_lgb)
            if cbs and w_cb > 0:
                preds.append(self._ensemble_predict_cb(cbs, feats))
                ws.append(w_cb)
            if not preds:
                continue
