"""Predictor for T166 v2NN_CB: 4-way ensemble + selective agreement filter.

Combines:
  1. T87 SPO+ DFL NN (nn_h60_seed*.npz)      w=1.5
  2. T97 multi-head NN (nn2_h60_seed*.npz)   w=1.5
  3. T75 LGB full-retrain (model_h60_seed*.txt) w=1.0
  4. CatBoost L2 full-retrain (model_CB_full_seed*.cbm) w=1.0

  pred = (w_nn*nn + w_nn2*nn2 + w_lgb*lgb + w_cb*cb) / (w_nn+w_nn2+w_lgb+w_cb)

Selective agreement filter (T158/T163 pattern):
  NN-side = weighted avg of T87+T97: (w_nn*nn + w_nn2*nn2) / (w_nn+w_nn2)
  For syms in {0,1,2}: if sign(NN-side) != sign(lgb): abstain (action=1)

Conformal wrapper (unchanged from T163):
...
class Predictor:
    def __init__(self) -> None:
        ...
        for hcfg in self._horizons:
            ...
            lgb_paths, nn_paths, nn2_paths, cb_paths = [], [], [], []
            for s in seeds:
    ...
    def predict(self, batches: List[pd.DataFrame]) -> List[List[int]]:
        ...
        for hcfg in self._horizons:
            ...
            w_nn, w_lgb, w_nn2, w_cb = self._weights.get(H, (1.0, 1.0, 0.0, 0.0))
            ...
            # Selective agreement filter (T158/T163 pattern, adapted for 4-way):
