...
Loads pred_T96_seed{S}.parquet for S in 1,7,13,42,100, averages predicted
...
def load_avg_pred(seeds):
    base = pd.read_parquet(os.path.join(HERE, f"pred_T96_seed{seeds[0]}.parquet"))
    p_sum = base["pred_dmid_norm"].to_numpy(np.float64).copy()
    n = len(base)
    for s in seeds[1:]:
        df_s = pd.read_parquet(os.path.join(HERE, f"pred_T96_seed{s}.parquet"))
        if len(df_s) != n:
