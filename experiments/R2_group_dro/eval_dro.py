"""R2 evaluation: DE asym thresh, per-sym PnL.

Loads R2 baseline + trick predictions, averages over 3 seeds {1,7,42},
runs DE asym threshold search on full 442k local test set, computes
per-sym PnL summary. Compares baseline vs trick (eta sweep).
...
T99_5SEED_DE_BASE = None  # we measure 3-seed in this experiment
...
def avg_preds(prefix):
    base = pd.read_parquet(os.path.join(HERE, f"pred_R2_{prefix}_seed{SEEDS[0]}.parquet"))
    p = np.zeros(len(base), dtype=np.float64)
    for s in SEEDS:
        df = pd.read_parquet(os.path.join(HERE, f"pred_R2_{prefix}_seed{s}.parquet"))
...
def main():
    print("=== R2 Group-DRO LGB Huber 3-seed eval ===", flush=True)
    t0 = time.time()
    base = evaluate("Baseline (eta=0)", "baseline")
    eta05 = evaluate("Trick (eta=0.5)", "eta0.5")
    eta10 = evaluate("Trick (eta=1.0)", "eta1")
    eta20 = evaluate("Trick (eta=2.0)", "eta2")

    by_eta = {"baseline": base, "eta0.5": eta05, "eta1.0": eta10, "eta2.0": eta20}

    best_trick = max([eta05, eta10, eta20], key=lambda r: r["de_sum"])
    delta_loso = best_trick["de_sum"] - base["de_sum"]
    delta_per_sym_min = best_trick["de_per_sym_min"] - base["de_per_sym_min"]
    ...
    print(f"BEST TRICK [{best_trick['label']}] DE sum={best_trick['de_sum']:+.4f}", flush=True)
    print(f"  per_sym = {[round(x,2) for x in best_trick['de_per_sym']]}", flush=True)
    print(f"  per_sym_min = {best_trick['de_per_sym_min']:.4f}", flush=True)
    ...
    out = {
        "task": "R2 Group-DRO over sym (LGB Huber)",
        "seeds": list(SEEDS),
        "etas": [0.0, 0.5, 1.0, 2.0],
        "by_eta": by_eta,
        "best_trick_label": best_trick["label"],
        "metrics": {
            "baseline_loso": base["de_sum"],
            "trick_loso_eta0.5": eta05["de_sum"],
            "trick_loso_eta1.0": eta10["de_sum"],
            "trick_loso_eta2.0": eta20["de_sum"],
            "trick_loso_best": best_trick["de_sum"],
            "delta_loso": delta_loso,
            "per_sym_min_baseline": base["de_per_sym_min"],
            "per_sym_min_trick_best": best_trick["de_per_sym_min"],
            "delta_per_sym_min": delta_per_sym_min,
            "per_sym_std_baseline": base["de_per_sym_std"],
            "per_sym_std_trick_best": best_trick["de_per_sym_std"],
        },
