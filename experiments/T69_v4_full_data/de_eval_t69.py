"""T69 DE 4D threshold search on 5-seed-averaged predictions.

Loads pred_T69_seed{S}.parquet for S in 1,7,13,42,100, averages probs across
...
def load_avg_pred(seeds):
    base = pd.read_parquet(os.path.join(HERE, f"pred_T69_seed{seeds[0]}.parquet"))
    p_sum = base[PROB_COLS].to_numpy(np.float64).copy()
    n = len(base)
    for s in seeds[1:]:
        df_s = pd.read_parquet(os.path.join(HERE, f"pred_T69_seed{s}.parquet"))
        if len(df_s) != n:
...
def main():
    ...
    print(f"=== T69 DE 4D thresh seeds={seeds} ===", flush=True)
    ...
    out = {
        "task": "T69 DE 4D thresh (V4 walk-fwd full data)",
        ...
        "compare_iter011": {
            ...
            "t69_de_single_total_vs_iter011": de_single_total - ITER011_LOSO,
            "t69_de_loso_sum_vs_iter011": de_loso_sum - ITER011_LOSO,
        },
    }
    out_path = os.path.join(HERE, "de_results_t69.json")
    ...
    print(f"vs iter_011 +{ITER011_LOSO:.2f}: t69_de_loso_diff={de_loso_sum - ITER011_LOSO:+.4f}", flush=True)
