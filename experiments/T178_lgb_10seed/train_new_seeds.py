"""T178: Train 5 new LGB seeds (200,211,222,233,244) using M7 trick (full data 0-119).

Based on R_full_retrain/train_full_memeff.py with GPU mode and new seed configs.
"""
...
SEED_CONFIGS = {
    200: dict(seed=200, feature_fraction=0.65, bagging_fraction=0.75, num_leaves=127, lambda_l2=1.5),
    211: dict(seed=211, feature_fraction=0.55, bagging_fraction=0.65, num_leaves=63,  lambda_l2=2.5),
    222: dict(seed=222, feature_fraction=0.75, bagging_fraction=0.85, num_leaves=255, lambda_l2=0.7),
    233: dict(seed=233, feature_fraction=0.45, bagging_fraction=0.55, num_leaves=127, lambda_l2=2.0),
    244: dict(seed=244, feature_fraction=0.85, bagging_fraction=0.95, num_leaves=63,  lambda_l2=0.5),
}
...
def main():
    ...
    H = args.horizon
    seed = args.seed
    assert seed in SEED_CONFIGS, f"seed {seed} not in SEED_CONFIGS: {list(SEED_CONFIGS.keys())}"
    print(f"=== T178 new seed training (memeff) seed={seed} h={H} num_boost={args.num_boost_round} ===", flush=True)
    ...
    summary = {
        "task": f"T178 new seed training seed={seed}",
        "seed": seed,
        "horizon": H,
        "n_features": feat_dim,
        "num_boost_round": args.num_boost_round,
