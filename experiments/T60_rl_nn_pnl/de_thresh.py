"""T60 DE 4D threshold search on T60 NN OOF predictions.

Loads NPZ predictions saved by train_pnl_nn.py:
    {prefix}_seed{S}_held{K}.npz  with fields:
        probs_te, y_te, mp_t_te, mp_th_te
averages probs across seeds per fold, then:
  1) Raw argmax baseline
  2) Coarse 4D grid sweep
  3) Differential evolution (4D continuous) on (T_up, T_dn, d_up, d_dn) with 5 DE seeds
  4) Local neighborhood sanity check

Writes thresh_results.json into HERE.
"""
...
def load_fold_avg_npz(prefix: str, k: int, seeds: List[int]):
    base = np.load(os.path.join(HERE, f"{prefix}_seed{seeds[0]}_held{k}.npz"))
    p_sum = base["probs_te"].astype(np.float64).copy()
    n = p_sum.shape[0]
    for s in seeds[1:]:
        d = np.load(os.path.join(HERE, f"{prefix}_seed{s}_held{k}.npz"))
        p_s = d["probs_te"].astype(np.float64)
        if p_s.shape[0] != n:
            raise RuntimeError(f"row count mismatch fold={k} seed={s}: {p_s.shape[0]} vs {n}")
        p_sum += p_s
    p_avg = p_sum / float(len(seeds))
    return dict(
        probs=p_avg.astype(np.float32),
        label=base["y_te"].astype(np.int64),
        mp_t=base["mp_t_te"].astype(np.float64),
        mp_th=base["mp_th_te"].astype(np.float64),
    )
...
def fold_arrays_from_dicts(folds):
    return [FoldArrays(fold=k, **folds[k]) for k in range(N_FOLDS)]


def coarse_sweep(fas, T_lo=0.40, T_hi=0.7001, T_step=0.02,
                 d_lo=0.0, d_hi=0.2001, d_step=0.02):
    T_up = np.round(np.arange(T_lo, T_hi, T_step), 4)
    T_dn = np.round(np.arange(T_lo, T_hi, T_step), 4)
    d_up = np.round(np.arange(d_lo, d_hi, d_step), 4)
    d_dn = np.round(np.arange(d_lo, d_hi, d_step), 4)
...
def neighborhood_check(fas, Tu0, Td0, du0, dd0, half_T=0.04, half_d=0.04, step=0.01):
    Tu_grid = np.round(np.arange(max(0.30, Tu0 - half_T), Tu0 + half_T + 1e-9, step), 4)
    Td_grid = np.round(np.arange(max(0.30, Td0 - half_T), Td0 + half_T + 1e-9, step), 4)
...
def main():
    ...
    ap.add_argument("--prefix", default="loso_pred_h60",
                    help="NPZ prefix: HERE/{prefix}_seed{S}_held{K}.npz")
    ap.add_argument("--seeds", default="42",
                    help="comma-separated seeds to average over")
    ap.add_argument("--out", default="thresh_results.json")
    args = ap.parse_args()

    seeds = [int(s) for s in args.seeds.split(",")]
    print(f"=== T60 DE thresh search prefix={args.prefix} seeds={seeds} ===", flush=True)

    t0 = time.time()
    folds = {k: load_fold_avg_npz(args.prefix, k, seeds) for k in range(N_FOLDS)}
    fas = fold_arrays_from_dicts(folds)
    n_total = sum(f["probs"].shape[0] for f in folds.values())
    print(f"  loaded {n_total:,} OOF rows in {time.time()-t0:.1f}s", flush=True)
    ...
    print("\n[1/3] Coarse 4D sweep ...", flush=True)
    t1 = time.time()
