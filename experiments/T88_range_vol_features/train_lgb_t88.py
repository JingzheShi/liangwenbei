"""T88 LGB regression: schemeP + 12 range-vol features (370 + 12 = 382 → 371 after drops).

Mirrors T75 train_regr.py with one change: concatenate T88 range-vol features
to schemeP X. Same V4 split, same SEED_CONFIGS, same aug_a, same loss.
"""
...
def load_split_concat(split: str) -> dict:
    """Load schemeP cache + t88 cache, return concatenated dict.
    X shape: (N, 370 + 12 = 382)."""
    ...
    X_sp = sp["X"]
    X_t88 = t88["X_t88"]
    assert X_sp.shape[0] == X_t88.shape[0], (X_sp.shape, X_t88.shape)
    # Sanity row alignment
    assert (sp["sym"] == t88["sym"]).all(), "sym mismatch"
    assert (sp["date"] == t88["date"]).all(), "date mismatch"
    assert (sp["sess_idx"] == t88["sess_idx"]).all(), "sess_idx mismatch"
    assert (sp["t"] == t88["t"]).all(), "t mismatch"
    X_full = np.concatenate([X_sp, X_t88.astype(np.float32)], axis=1)
...
def build_v4_split(train_full, slicer):
    ...
    info = {
        "strategy": "V4",
        "train_dates": "0-75", "val_dates": "76-79 (walk-forward last 5%)",
...
def main():
    ...
    H = args.horizon
    seed = args.seed
    print(f"=== T88 LGB regression Δmid seed={seed} h={H} ===", flush=True)
    ...
    assert total_dim == 382, total_dim
    ...
    print(f"  feat_dim={feat_dim} (dropped {len(drop_idx)}); + {len(t88_names)} new t88", flush=True)
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = f"T88-LGB-seed{seed}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            wandb.init(project="liangwenbei", entity="cjxh21-Tsinghua University",
                       name=run_name, config={"task": "T88_range_vol", "seed": seed,
                                              "n_features": feat_dim,
                                              "horizon": H, "model": "lgb-gpu",
                                              **{k: v for k, v in vars(args).items()}},
                       tags=["T88", "range_vol", "lgb", f"h{H}"])
        except Exception as e:
            print(f"  WandB init failed ({e!r}); skipping"); use_wandb = False
    ...
    model_path = os.path.join(HERE, f"model_T88lgb_seed{seed}.txt")
    ...
    pred_path = os.path.join(HERE, f"pred_T88lgb_seed{seed}.parquet")
    ...
    pred_action[te_pred > fee_thr] = 2; pred_action[te_pred < -fee_thr] = 0
    ...
    print(f"  TEST EV-gate k=1: cum_pnl={pnl.sum():+.4f} n_active={(pred_action!=1).sum():,}",
          flush=True)

    summary = {
        "task": f"T88 LGB regression seed={seed}", "seed": seed, "horizon": H,
        "n_features": feat_dim, "drop_names": list(DROP_NAMES),
        "best_iter": best_iter, "train_time_sec": float(train_time),
        "val": {"mse": va_mse, "corr": va_corr},
        "test": {"mse": te_mse, "corr": te_corr,
                 "ev_gate_k1_cum_pnl": float(pnl.sum())},
        "params": vars(args),
    }
    summ_path = os.path.join(HERE, f"summary_T88lgb_seed{seed}.json")
    ...
    progress("done", seed=seed, val_mse=va_mse, test_corr=te_corr)
