"""Predictor V3_adaptive_thr: T168 per-batch confidence-scaled predictions variant.

V3: scale pred by within-batch relative confidence (|pred| / q75_abs), clipped to [0,1].
Lower-confidence rows get their pred shrunk toward 0 before applying the static gate.
Conformal band applied on top of static thresholds.
...
class Predictor:
    ...
    def predict(self, batches: List[pd.DataFrame]) -> List[List[int]]:
        ...
        for hcfg in self._horizons:
            ...
            # V3: scale pred by within-batch relative confidence, then apply original gate
            ...
            out_h[pred_scaled > (thr_up + band_per_row)] = 2
            out_h[pred_scaled < -(thr_dn + band_per_row)] = 0
