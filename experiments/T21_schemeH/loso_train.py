"""T21: Multi-seed LOSO LightGBM trainer for Scheme H (291-d, h=10 by default).

Identical pipeline to T11_schemeC_multiseed/loso_train.py but with:
  - Scheme H 294-col cache (drops trailing 3 time-encoding cols → 291-d train)
  - GPU LightGBM (3.2x faster vs CPU per CLAUDE.md)

Default 5 fold × 5 seeds × h=10 = 25 fits. With GPU each ≈ 1-2 min on 1.5M × 291.

Outputs (per (seed, horizon, fold)):
    loso_pred_h{H}_seed{S}_held{K}.parquet
    loso_model_h{H}_seed{S}_held{K}.txt
"""
...
T21_CACHE = os.path.join(HERE, "cache")
NUM_CLASS = 3
SYMS = (0, 1, 2, 3, 4)
N_DROP_TAIL = 3  # last 3 cols are time encoding

# Same diversity-forcing seed configs as T11.
SEED_CONFIGS = {
...
def load_split(split):
    p = os.path.join(T21_CACHE, f"schemeH_{split}.npz")
...
def train_one(H, seed_cfg, held, train_full, val_full, test_full, feat_names, args):
    ...
    # feature importance (gain) for analysis
...
def main():
    ...
    ap.add_argument("--out-tag", default="T21")
    args = ap.parse_args()
    if args.no_gpu:
        args.use_gpu = False
    ...
    print(f"=== T21 LOSO Scheme H {291}-d, GPU={args.use_gpu}, "
          f"horizons={horizons}, seeds={seeds}, syms={target_syms} ===", flush=True)
    ...
    feat_names_path = os.path.join(T21_CACHE, "schemeH_no_time_feat_names.txt")
    ...
    assert len(feat_names) == train_full["X"].shape[1] - N_DROP_TAIL, \
        f"feat_names {len(feat_names)} != X cols {train_full['X'].shape[1] - N_DROP_TAIL}"
    ...
    if use_wandb:
        try:
            ...
            wandb = wandb_mod
            run_name = f"T21-schemeH-top15-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            init_kwargs = dict(
                project="liangwenbei",
                name=run_name,
                config={
                    "model": "lightgbm-gpu" if args.use_gpu else "lightgbm-cpu",
                    "scheme": "H-291d",
                    "n_features": len(feat_names),
                    "n_features_total": 291,
                    "n_features_r20": 68,
                    "horizons": horizons,
                    "seeds": seeds,
                ...
                tags=["T21", "schemeH", "r20-top15", "loso"],
            )
            try:
    ...
    for H in horizons:
        ...
        for s in seeds:
            ...
            for held in target_syms:
                ...
                progress("training", horizon=H, seed=s, held=held, done=done, total=n_total)
                r = train_one(H, seed_cfg, held, train_full, val_full, test_full, feat_names, args)
    ...
    with open(summary_path, "w") as f:
        # drop heavy feat_imp arrays from summary file (kept inside boosters)
        compact = []
        for r in all_results:
            cr = {k: v for k, v in r.items() if not k.startswith("feat_imp_")}
            compact.append(cr)
        json.dump(sanitize_for_json({
            "task": "T21 multi-seed LOSO Scheme H 291-d (R20 Top-15)",
    ...
    # Save aggregated feature importance (mean over folds × seeds)
    if all_results:
        ...
        fi_path = os.path.join(HERE, "feature_importance_avg.csv")
        fi_df.to_csv(fi_path, index=False)
        print(f"feature importance -> {fi_path}", flush=True)
        print("\n=== TOP 30 BY GAIN ===", flush=True)
        for i, row in fi_df.head(30).iterrows():
            print(f"  {row['feat_name']:38s}  gain={row['gain_sum_mean']:.0f}  split={row['split_count_mean']:.0f}",
                  flush=True)
