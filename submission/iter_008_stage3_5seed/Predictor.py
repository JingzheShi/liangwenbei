"""Predictor for iter_008: T53 R34 Stage 1+2+3 features (340-d) with 5-seed full-data
LightGBM ensemble for h_60, plus T53b asymmetric (T_up,T_dn,d_up,d_dn) DE-optimized
gating tuned on T53 5-seed LOSO OOF.
...
Feature pipeline per inference call:
  ...
  --------------------------------------------------------------------------------
  total 350, then drop 10 FAIL extras -> 340 final features (matches model.feature_name())
...
class Predictor:
    def __init__(self) -> None:
        ...
        self._t3 = _load_module(here, "compute.py", "iter_008_t3_compute")
        self._s1 = _load_module(here, "r34_features.py", "iter_008_r34_s1")
        self._s2 = _load_module(here, "r34_stage2_features.py", "iter_008_r34_s2")
        self._s3 = _load_module(here, "r34_stage3_features.py", "iter_008_r34_s3")
        ...
        s3_names = list(self._s3.all_feature_names())
        full_extra_names = s1_names + s2_names + s3_names
    ...
    def _compute_window_features(self, df: pd.DataFrame) -> np.ndarray:
        ...
        # R34 stage1/2/3 expect df["midprice"] (= midprice1) and
        # df["mlofi_W20_lvl_k"] (k=1,5,10) computed by T3 above.
        ...
        s3 = self._s3.compute_all_session(df_ext, last, last)
        extras = np.concatenate([s1, s2, s3], axis=1).reshape(-1)
