"""iter_010 batch-vectorized feature computation.
...
a (N, 196) feature block: T3-no-time(69) + Stage1(54) + Stage2(59) + Stage3(14).
...
def compute_batch_features(X3d: np.ndarray, col_idx: Dict[str, int]) -> np.ndarray:
    """Compute the (T3+S1+S2+S3) feature block for N windows.
    ...
    return np.concatenate([t3_v, s1_v, s2_v, s3_v], axis=1)
