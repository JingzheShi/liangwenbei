...
| 001f | 2026-05-06 | LightGBM **Scheme D1** (raw 154 + window-zscore 154 = **308 features**) + threshold (T=0.45, δ=0.05) | sym-agnostic z-score normalization 修 sym=2 brittleness（per T8 诊断 F1） | **LOSO +13.91** (**5/5 folds positive!**) | label_60 | _待提交_ | LOSO +1.74e-04 | LOSO 0.615 | sym=2 从 -0.98 翻到 **+2.24**！比 iter_001d 高 +2.80；`submission_050605_iter1f.zip` 1.89MB / 22 sanity ✓ |
| 002  | 2026-05-06 | LightGBM **Scheme C** (T2 154 last-tick + T3 72 features = 226d) **× 5 horizons 各一个独立模型** + per-horizon thresholds | multi-horizon 5 head；每个 horizon 独立训练 + 独立调 threshold | **LOSO h_10 +21.86, h_20 +19.70, h_5 +17.49, h_40 +11.71, h_60 +6.30** | LOSO best h_10 / **平台 best h_60 = +4.07** | 任务 2435 ✓ 5 horizons: -7.68/-8.64/-5.09/+2.02/**+4.07** | h_60 +0.000117 (正 alpha!) | label_60 0.209 | **🏆 平台首次正分**：+4.07 vs iter_000 -6.65 → +10.72 提升；label_40+60 都正；h_60 LOSO+6.30 → 平台+4.07 校准良好；h_10 LOSO+21.86 → 平台-8.64 严重过拟合 |
| 003  | 2026-05-06 | iter_002 + **h_10 替换为 5-seed Scheme C ensemble**（seeds 42/1/7/13/100，强制 hyperparam diversity） | T11 worker 多 seed average prob + threshold | **LOSO h_10 +22.22** (+0.36 over iter_002) / 其他 4 horizon 同 iter_002 | h_10 | _待提交_ | h_10 LOSO single ~+1.5e-04 | h_10 LOSO 0.62 | **🏆 当前最强**：5/5 folds positive；`submission_050614_iter003.zip` 34.59MB / 22 sanity ✓ |
