    ...
    def predict(self, batches: List[pd.DataFrame]) -> List[List[int]]:
        ...
        for hcfg in self._horizons:
            if not hcfg.get("active", True):
                continue
            H = int(hcfg["h"])
            # Resolve which horizon's models to use (share_with allows reuse of another horizon's ensemble)
            src_H = int(hcfg.get("share_with", H))

            if src_H in pred_cache:
                pred_dmid = pred_cache[src_H]
            else:
                lgbs = self._lgb_lists.get(src_H)
                nn_ens = self._nn_batched.get(src_H)
                w_nn, w_lgb = self._weights.get(src_H, (1.0, 1.0))
