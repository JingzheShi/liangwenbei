"""V16 trainer: combine v15 winners (decoupled_lr + per_sym_bias) ± 80feat.

V16.A  combine_bd       — decoupled lr + per_sym_bias (60 feat)
V16.B  lr_sweep_5e6     — decoupled lr 5e-6 / 3e-4 (ratio 1:60), 60 feat
...
Reads model.lr_encoder/.lr_head/.lr_other when model.lr_groups is True; falls
back to v15 defaults if unset (1e-5 / 3e-4 / 1e-4).
"""
...
from models_v16 import build_model_v16, count_params
...
def _build_optimizer_decoupled(model, lr_encoder, lr_head, lr_other, weight_decay):
    ...
    print(f'  [decoupled] encoder={sum(p.numel() for p in enc_params):,} (lr={lr_encoder}) '
          f'head={sum(p.numel() for p in head_params):,} (lr={lr_head}) '
          f'other={sum(p.numel() for p in other_params):,} (lr={lr_other})', flush=True)
    return opt


def train_v16(seed, device, keep_idx_npy, sel_idx_npy, zstats_npz,
              arch='v16_a_combine_bd', out_tag='v16_arch',
    ...
    print(f'\n=== [v16] arch={arch} seed={seed} keep_259={len(keep_idx)} sel_inter={len(sel_idx)} ===',
          flush=True)
    n_keep = len(keep_idx)
    n_sel  = len(sel_idx)
        ...
        try:
            ...
            wandb.init(project=wandb_project, entity='jingzheshi', name=run_name,
                       config=dict(arch=arch, seed=seed, n_feat=n_feat,
                                   ...
                                   ablation='v16_combine', mirror_aug=False, scale_jitter=False,
    ...
    model = build_model_v16(arch, n_sym=5, n_feat=n_feat, **overrides).to(device)
    n_params = count_params(model)
    use_decoupled = getattr(model, 'lr_groups', False)
    has_sym_bias = hasattr(model, 'sym_bias')
    lr_encoder = getattr(model, 'lr_encoder', 1e-5)
    lr_head_   = getattr(model, 'lr_head', 3e-4)
    lr_other_  = getattr(model, 'lr_other', 1e-4)
    print(f'  n_params={n_params:,}  decoupled_lr={use_decoupled} sym_bias={has_sym_bias}', flush=True)
    if use_wandb:
        import wandb
        wandb.config.update({'n_params': int(n_params),
                             'use_decoupled_lr': bool(use_decoupled),
                             'has_sym_bias': bool(has_sym_bias),
                             'lr_encoder': float(lr_encoder),
                             'lr_head': float(lr_head_),
                             'lr_other': float(lr_other_)},
                            allow_val_change=True)

    if use_decoupled:
        opt = _build_optimizer_decoupled(model, lr_encoder=lr_encoder, lr_head=lr_head_,
                                         lr_other=lr_other_, weight_decay=weight_decay)
    else:
...
def main():
    ...
    ap.add_argument('--arch', type=str, default='v16_a_combine_bd')
    ...
    ap.add_argument('--out-root', type=str, default='runs_v16')
    ap.add_argument('--out-tag', type=str, default='v16_arch')
    args = ap.parse_args()
