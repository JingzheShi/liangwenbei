"""V5 training: deeper / gated / wider / multi-block cross-sym attention.

Identical training protocol to train_v4.py for sae_cross_sym (the baseline):
  - 259d pruned features (keep_idx_259d.npy)
  - bs=1024, max_epochs=20
  - AdamW lr=1e-4, wd=1e-3, warmup_steps=200, cosine to eta_ratio=0.05
  - step-level eval every 200, early stop patience 1500 steps
  - mirror augment 5 sym already in data_loader
  - h=60 PnL on the canonical test split

Only difference: --arch comes from models_v5 (V1/V2/V3/V5 variants).

CLI: python train_v5.py --arch v1_deeper2 --seed 0 --keep-idx keep_idx_259d.npy
"""
...
from models_v5 import build_model_v5, count_params, ARCH_CLASSES_V5
from models_v5_b import build_model_v5b, ARCH_CLASSES_V5B, ARCH_DEFAULTS_V5B

# Merged arch registry
_ALL_ARCH_CLASSES = {**ARCH_CLASSES_V5, **ARCH_CLASSES_V5B}

def _build_model(arch: str, n_sym: int = 5, n_feat: int = 259):
    if arch in ARCH_CLASSES_V5B:
        return build_model_v5b(arch, n_sym=n_sym, n_feat=n_feat)
    return build_model_v5(arch, n_sym=n_sym, n_feat=n_feat)


def train_one_v5(arch, seed, device, keep_idx_npy,
    ...
    print(f"\n=== [v5] arch={arch} seed={seed} PRUNED keep={n_keep} ===", flush=True)
    ...
    print(f"  after prune: train={train_data['X_grp'].shape} val={val_data['X_grp'].shape} test={test_data['X_grp'].shape}", flush=True)
    ...
    model = _build_model(arch, n_sym=5, n_feat=n_feat).to(device)
    ...
    for ep in range(max_epochs):
        ...
        for s_idx in range(0, N, batch_size):
            ...
            if model._last_aux_loss is not None:
                aux_scale = getattr(model, 'mask_alpha', 1.0)
...
def main():
    ...
    ap.add_argument("--arch", type=str, required=True,
                    choices=list(_ALL_ARCH_CLASSES.keys()))
    ...
    ap.add_argument("--out-root", type=str, default="runs_v5")
    args = ap.parse_args()
