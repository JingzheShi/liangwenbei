"""V7 aug ablation A: NARROW scale jitter U(0.95, 1.05) instead of U(0.80, 1.20).

All else identical to train_v7.py.
"""
...
def train_v7_aug_narrow(seed, device, keep_idx_npy, sel_idx_npy, zstats_npz,
                        arch='v6_pairwise', out_tag='aug_narrow',
    ...
    print(f'\n=== [v7_aug_narrow] arch={arch} seed={seed} keep_259={len(keep_idx)} sel_inter={len(sel_idx)} ===',
          flush=True)
    n_keep = len(keep_idx)
    n_sel  = len(sel_idx)
        ...
        try:
            ...
            wandb.init(project=wandb_project, entity='jingzheshi', name=run_name,
                       config=dict(arch=arch, seed=seed, n_feat=n_feat,
                                   ...
                                   ablation='aug_narrow', scale_range='0.95-1.05',
    ...
    for ep in range(max_epochs):
        ...
        for s_idx in range(0, N, batch_size):
            ...
            # ABLATION A: narrow jitter U(0.95, 1.05) instead of U(0.80, 1.20)
...
def main():
    ...
    ap.add_argument('--out-tag', type=str, default='aug_narrow')
    args = ap.parse_args()
