"""V7 aug ablation C: PURE L2 — scale=1.0, no jitter at all.
...
def train_v7_pure_l2(seed, device, keep_idx_npy, sel_idx_npy, zstats_npz,
                     arch='v6_pairwise', out_tag='pure_l2',
    ...
    print(f'\n=== [v7_pure_l2] arch={arch} seed={seed} keep_259={len(keep_idx)} sel_inter={len(sel_idx)} ===',
          flush=True)
    n_keep = len(keep_idx)
    n_sel  = len(sel_idx)
        ...
        try:
            ...
            wandb.init(project=wandb_project, entity='jingzheshi', name=run_name,
                       config=dict(arch=arch, seed=seed, n_feat=n_feat,
                                   ...
                                   ablation='pure_l2', scale_range='1.0',
    ...
    for ep in range(max_epochs):
        ...
        for s_idx in range(0, N, batch_size):
            ...
            # ABLATION C: pure L2, no scale jitter
            scale = 1.0
            preds = model(xb)
            sq = (preds - yb * scale) ** 2
...
def main():
    ...
    ap.add_argument('--out-tag', type=str, default='pure_l2')
    args = ap.parse_args()
