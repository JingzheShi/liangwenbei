"""V13 trainer: supports 5 new directions building on V12.B inverted SOTA.

Differences from train_v12_arch.py:
  * V13.A (spo_plus loss): if arch == 'v13_a_spo', replace MSE with SPO+ surrogate.
  * V13.C (80 features):  caller supplies --sel-idx selected_features_v9_80.npy.
  * V13.D (contrastive):  model.forward sets _last_aux_loss; trainer adds with aux_alpha.
  * V13.E (multi-horizon): uses load_grouped_data_mh (5 targets), sums per-horizon MSE.
                           Eval & test still on h=60 only (main horizon).
"""
...
from data_loader_mh import load_grouped_data_mh, HORIZONS
from models_v13 import build_model_v13, count_params, spo_plus_loss
...
def train_v13(seed, device, keep_idx_npy, sel_idx_npy, zstats_npz,
              arch='v13_b_gated', out_tag='v13_arch',
    ...
    print(f'\n=== [v13] arch={arch} seed={seed} keep_259={len(keep_idx)} sel_inter={len(sel_idx)} ===',
          flush=True)
    n_keep = len(keep_idx)
    n_sel  = len(sel_idx)

    is_multih = (arch == 'v13_e_multihorizon')

    print('  loading data ...', flush=True)
    if is_multih:
        train_data, val_data, test_data, stats = load_grouped_data_mh(level='L8', verbose=True)
        target_scale_d = stats['target_scale_d']
        target_scale = stats['target_scale']  # h=60 scale, for inference
    else:
    ...
    if is_multih:
        for h in HORIZONS:
            train_data[f'y_reg_h{h}'] = train_data[f'y_reg_h{h}'][:N_orig]
    ...
    # multi-horizon targets (scaled per-h)
    ytr_h_t = None
    if is_multih:
        ytr_h_t = {h: torch.from_numpy(
            (train_data[f'y_reg_h{h}'] / target_scale_d[h]).astype(np.float32))
            for h in HORIZONS}
        ...
        try:
            ...
            wandb.init(project=wandb_project, entity='jingzheshi', name=run_name,
                       config=dict(arch=arch, seed=seed, n_feat=n_feat,
                                   ...
                                   ablation='v13_explore', mirror_aug=False, scale_jitter=False,
    ...
    model = build_model_v13(arch, n_sym=5, n_feat=n_feat, **overrides).to(device)
    n_params = count_params(model)
    use_spo = (getattr(model, 'loss_kind', None) == 'spo_plus')
    print(f'  n_params={n_params:,}  is_multih={is_multih}  use_spo={use_spo}', flush=True)
    if use_wandb:
        import wandb
        wandb.config.update({'n_params': int(n_params), 'use_spo': use_spo},
    ...
    for ep in range(max_epochs):
        ...
        for s_idx in range(0, N, batch_size):
            ...
            preds = model(xb)   # for multih: returns h=60 head, stores all in _last_preds_dict

            if use_spo:
                pred_loss = spo_plus_loss(preds, yb, wb, fee=1e-4)
            else:
                sq = (preds - yb) ** 2
                pred_loss = (sq * wb).sum() / wb.sum().clamp_min(1.0)

            # multi-horizon: add per-horizon MSE on heads (using own target_scale)
            if is_multih:
                pd = model._last_preds_dict   # dict[h -> (B, S)]
                aux_h_total = 0.0
                n_aux = 0
                for h in HORIZONS:
                    if h == 60:
                        continue   # already in pred_loss
                    yb_h = ytr_h_t[h][idx].to(device, non_blocking=True)
                    p_h = pd[h]
                    sq_h = (p_h - yb_h) ** 2
                    pl_h = (sq_h * wb).sum() / wb.sum().clamp_min(1.0)
                    aux_h_total = aux_h_total + pl_h
                    n_aux += 1
                pred_loss = pred_loss + aux_h_total / max(n_aux, 1)
    ...
    result = {
        ...
        'ablation': 'v13_explore', 'mirror_aug': False, 'scale_jitter': False,
        'is_multih': bool(is_multih), 'use_spo': bool(use_spo),
        'n_train_groups': int(N_orig),
...
def main():
    ...
    ap.add_argument('--arch', type=str, default='v13_b_gated')
    ...
    ap.add_argument('--out-root', type=str, default='runs_v13')
    ap.add_argument('--out-tag', type=str, default='v13_arch')
    args = ap.parse_args()
