"""V6 training: 5 paradigm-shift archs (GAT, Pairwise, CrossFeat, Bottleneck, MoE).

Identical training protocol to train_v5.py for sae_cross_sym (the baseline):
...
CLI: python train_v6.py --arch v6_gat --seed 0 --keep-idx keep_idx_259d.npy
"""
...
from models_v6 import build_model_v6, count_params, ARCH_CLASSES_V6, ARCH_DEFAULTS_V6


def _build_model(arch: str, n_sym: int = 5, n_feat: int = 259, **overrides):
    return build_model_v6(arch, n_sym=n_sym, n_feat=n_feat, **overrides)


def train_one_v6(arch, seed, device, keep_idx_npy,
    ...
    print(f"\n=== [v6] arch={arch} seed={seed} PRUNED keep={n_keep} ===", flush=True)
    ...
    model = _build_model(arch, n_sym=5, n_feat=n_feat, **overrides).to(device)
...
def main():
    ...
    ap.add_argument("--arch", type=str, required=True,
                    choices=list(ARCH_CLASSES_V6.keys()))
    ...
    ap.add_argument("--dropout", type=float, default=None,
                    help="Override default dropout for the arch (e.g. 0.35 / 0.40 for big archs)")
    ap.add_argument("--no-wandb", action="store_true")
    ap.add_argument("--wandb-project", type=str, default="liangwenbei-cross-sym")
    ap.add_argument("--out-root", type=str, default="runs_v6")
    args = ap.parse_args()
