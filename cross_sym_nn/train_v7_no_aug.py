"""V7 aug ablation B: NO bid/ask mirror augmentation (train on original only).

data_loader.py doubles train via mirror; here we truncate back to the original half.
All other training identical to train_v7.py.
"""
...
def train_v7_no_mirror(seed, device, keep_idx_npy, sel_idx_npy, zstats_npz,
                       arch='v6_pairwise', out_tag='no_mirror',
    ...
    print(f'\n=== [v7_no_mirror] arch={arch} seed={seed} keep_259={len(keep_idx)} sel_inter={len(sel_idx)} ===',
          flush=True)
    n_keep = len(keep_idx)
    n_sel  = len(sel_idx)
    ...
    # ABLATION B: discard mirror half — keep only original (first half)
    N_aug = X_tr.shape[0]
    N_orig = N_aug // 2
    X_tr = X_tr[:N_orig]
    train_data['y_reg'] = train_data['y_reg'][:N_orig]
    train_data['sw']    = train_data['sw'][:N_orig]
    print(f'  [no_mirror] truncated train: {N_aug} → {N_orig} groups', flush=True)
    ...
    run_name = f'{arch}_v7inter{n_sel}_{out_tag}_s{seed}'
    if use_wandb:
        try:
            import wandb
            ...
            wandb.init(project=wandb_project, entity='jingzheshi', name=run_name,
                       config=dict(arch=arch, seed=seed, n_feat=n_feat,
                                   ...
                                   ablation='no_mirror', mirror_aug=False,
    ...
    for ep in range(max_epochs):
        ...
        for s_idx in range(0, N, batch_size):
            ...
            # Keep original scale jitter U(0.80, 1.20)
            scale = float(1.0)
            preds = model(xb)
            sq = (preds - yb * scale) ** 2
    ...
    result = {
        ...
        'ablation': 'no_mirror', 'mirror_aug': False, 'n_train_groups': int(N_orig),
...
def main():
    ...
    ap.add_argument('--out-root', type=str, default='runs_v8')
    ap.add_argument('--out-tag', type=str, default='no_mirror')
    args = ap.parse_args()
