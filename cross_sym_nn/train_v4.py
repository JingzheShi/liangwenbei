"""V4 training: explore 4 NN variants on top of sae_mlp 259d SOTA.

Reuses the v2_pruned step-level training protocol exactly. Differences:
  - --arch ∈ {multi_h_sae, sae_cross_sym, sae_wider, vae_mlp}
  - multi_h_sae uses multi-horizon targets via data_loader_mh
  - vae_mlp adds beta_kl * KL term to the loss
  - Same eval protocol (h=60 PnL / IC) so results are directly comparable to
    runs_v2_pruned/sae_mlp_pruned259_s* (the SOTA baseline).

CLI: python train_v4.py --arch multi_h_sae --seed 0 --keep-idx keep_idx_259d.npy
"""
...
from models_v4 import build_model_v4, count_params, MultiHorizonSAEMLP, VAEMLP


# Default horizon weights for V1 — h=60 (main) gets weight 1.0, shorter horizons
# get smaller weights since their signal is noisier and lower-magnitude.
DEFAULT_H_WEIGHTS = {5: 0.2, 10: 0.3, 20: 0.4, 40: 0.6, 60: 1.0}


def train_one_v4(arch, seed, device, keep_idx_npy,
    ...
    print(f"\n=== [v4] arch={arch} seed={seed} PRUNED keep={n_keep} ===", flush=True)
    ...
    is_multi_h = (arch == "multi_h_sae")
    is_vae     = (arch == "vae_mlp")

    if is_multi_h:
        # per-horizon target tensors, each normalized by its own target_scale
        y_tr_h = {h: train_data[f'y_reg_h{h}'] / stats['target_scale_d'][h] for h in HORIZONS}
        h_weights = DEFAULT_H_WEIGHTS
    else:
        ...
        try:
            ...
            wandb.init(
                ...
                config=dict(arch=arch, seed=seed, n_feat=n_feat,
                            ...
                            train_loop="step_level_v4",
                            is_multi_h=is_multi_h, is_vae=is_vae),
    ...
    model = build_model_v4(arch, n_sym=5, n_feat=n_feat).to(device)
    n_params = count_params(model)
    print(f"  n_params={n_params:,}  is_multi_h={is_multi_h}  is_vae={is_vae}", flush=True)
    if use_wandb:
        import wandb
    ...
    if is_multi_h:
        ytr_t_h = {h: torch.from_numpy(y_tr_h[h].astype(np.float32)) for h in HORIZONS}
    ...
    for ep in range(max_epochs):
        ...
        for s_idx in range(0, N, batch_size):
            ...
            if is_multi_h:
                _ = model(xb)  # populates model._last_preds_multi & recon
                preds_multi = model._last_preds_multi  # list of (B,5) tensors
                pred_loss = 0.0
                w_sum = 0.0
                for i, h in enumerate(HORIZONS):
                    yb_h = ytr_t_h[h][idx].to(device, non_blocking=True)
                    sq = (preds_multi[i] - yb_h * scale) ** 2
                    pl_h = (sq * wb).sum() / wb.sum().clamp_min(1.0)
                    w = DEFAULT_H_WEIGHTS[h]
                    pred_loss = pred_loss + w * pl_h
                    w_sum += w
                pred_loss = pred_loss / w_sum  # normalize to same scale as single-h
                ...
                if is_vae and model._last_kl is not None:
                    loss = loss + model.beta_kl * model._last_kl
...
def main():
    ...
    ap.add_argument("--arch", type=str, default="multi_h_sae",
                    choices=["multi_h_sae", "sae_cross_sym", "sae_wider", "vae_mlp"])
    ...
    ap.add_argument("--out-root", type=str, default="runs_v4")
    args = ap.parse_args()
