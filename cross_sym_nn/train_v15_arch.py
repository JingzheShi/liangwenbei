"""V15 trainer: 5 fundamentally new directions building on V12.B (+40.39 SOTA).

V15.A — temporal-agg features (80 feat: 60 inter + 20 new); arch unchanged
V15.B — decoupled LR: encoder slow (1e-5), head fast (3e-4), others 1e-4
V15.C — SAM: two-pass sharpness-aware step (rho=0.05)
V15.D — per-sym output bias added to pred
V15.E — TTA: train standard, at final inference run N=10 noisy fwd & average
"""
...
from train_v2 import eval_predictions, eval_test, predict_grouped, make_lr_lambda
from data_loader import load_grouped_data
from models_v15 import build_model_v15, count_params
from interaction_features_v8 import compute_v9_features
from interaction_features_v15 import compute_v15_features


def _build_inter_features(X_359, sel_idx, mu, sd, arch, batch=200_000):
    """Build interaction features. arch determines which compute_*_features to call.

    v15_a_temporal → compute_v15_features (156 channels)
    else           → compute_v9_features  (136 channels)
    """
    use_v15 = (arch == 'v15_a_temporal')
    out_chunks = []
    n = X_359.shape[0]
    for s in range(0, n, batch):
        Xs = X_359[s:s + batch]
        if use_v15:
            Xi, _, _, _ = compute_v15_features(Xs)
        else:
...
def _build_optimizer_v15b(model, lr_encoder=1e-5, lr_head=3e-4, lr_other=1e-4,
                          weight_decay=1e-3):
    """V15.B param-group split: encoder slow, pred_head fast, others mid."""
    ...
    for name, p in model.named_parameters():
        ...
        if name.startswith('encoder.'):
            enc_params.append(p)
        elif name.startswith('pred_head.'):
    ...
    print(f'  [V15.B] param groups: encoder={sum(p.numel() for p in enc_params):,} (lr={lr_encoder}) '
          f'head={sum(p.numel() for p in head_params):,} (lr={lr_head}) '
          f'other={sum(p.numel() for p in other_params):,} (lr={lr_other})', flush=True)
    return opt


def _sam_step(model, opt, loss_fn, xb, yb, wb, rho=0.05, grad_clip=1.0):
    """SAM step: forward + ascent + forward + descent.

    Returns the loss value at the original params (first pass) for logging.
    """
    # First pass — compute grad
    preds = model(xb)
    sq = (preds - yb) ** 2
    pred_loss = (sq * wb).sum() / wb.sum().clamp_min(1.0)
    if getattr(model, '_last_recon_loss', None) is not None:
        alpha = model.recon_alpha
        loss1 = (1.0 - alpha) * pred_loss + alpha * model._last_recon_loss
    else:
        loss1 = pred_loss
    opt.zero_grad()
    loss1.backward()

    # Compute perturbation e = rho * grad / (||grad|| + eps)
    grad_norm = 0.0
    ...
    # Second pass — compute grad at perturbed point
    preds2 = model(xb)
    sq2 = (preds2 - yb) ** 2
    pred_loss2 = (sq2 * wb).sum() / wb.sum().clamp_min(1.0)
    if getattr(model, '_last_recon_loss', None) is not None:
        alpha = model.recon_alpha
        loss2 = (1.0 - alpha) * pred_loss2 + alpha * model._last_recon_loss
    else:
        loss2 = pred_loss2
    opt.zero_grad()
    loss2.backward()
    ...
    # Clip + step (using grad from perturbed point)
    torch.nn.utils.clip_grad_norm_(model.parameters(), grad_clip)
    opt.step()
    return float(loss1.item())


def _predict_tta(model, X_grp, device, n_aug=10, batch_size=1024):
    """TTA: run N forward passes with model.train() (re-enables input noise +
    dropout) and average predictions. Returns numpy (N, 5).
    """
    model.train()
    accum = None
    n = X_grp.shape[0]
    with torch.no_grad():
        for _ in range(n_aug):
            ...
            for s in range(0, n, batch_size):
                xb = torch.from_numpy(X_grp[s:s + batch_size]).to(device)
                yhat = model(xb).detach().cpu().numpy()
                preds_list.append(yhat)
            preds_np = np.concatenate(preds_list, axis=0).astype(np.float32)
            accum = preds_np if accum is None else (accum + preds_np)
    model.eval()
    return (accum / float(n_aug)).astype(np.float32)


def train_v15(seed, device, keep_idx_npy, sel_idx_npy, zstats_npz,
              arch='v15_a_temporal', out_tag='v15_arch',
    ...
    print(f'\n=== [v15] arch={arch} seed={seed} keep_259={len(keep_idx)} sel_inter={len(sel_idx)} ===',
          flush=True)
    n_keep = len(keep_idx)
    n_sel  = len(sel_idx)
    ...
    print(f'  computing inter features for arch={arch} ...', flush=True)
    t1 = time.time()
    X_tr_inter = _build_inter_features(train_data['X_grp'], sel_idx, mu_inter, sd_inter, arch)
    X_va_inter = _build_inter_features(val_data['X_grp'],   sel_idx, mu_inter, sd_inter, arch)
    X_te_inter = _build_inter_features(test_data['X_grp'],  sel_idx, mu_inter, sd_inter, arch)
        ...
        try:
            ...
            wandb.init(project=wandb_project, entity='jingzheshi', name=run_name,
                       config=dict(arch=arch, seed=seed, n_feat=n_feat,
                                   ...
                                   ablation='v15_explore', mirror_aug=False, scale_jitter=False,
    ...
    model = build_model_v15(arch, n_sym=5, n_feat=n_feat, **overrides).to(device)
    n_params = count_params(model)
    use_decoupled = getattr(model, 'lr_groups', False)
    use_sam = getattr(model, 'use_sam', False)
    sam_rho = float(getattr(model, 'sam_rho', 0.05))
    use_tta = getattr(model, 'use_tta', False)
    tta_n = int(getattr(model, 'tta_n', 10))
    print(f'  n_params={n_params:,}  decoupled_lr={use_decoupled}  sam={use_sam}  tta={use_tta}', flush=True)
    if use_wandb:
        import wandb
        wandb.config.update({'n_params': int(n_params),
                             'use_decoupled_lr': bool(use_decoupled),
    ...
    if use_decoupled:
        opt = _build_optimizer_v15b(model, lr_encoder=1e-5, lr_head=3e-4,
                                    lr_other=1e-4, weight_decay=weight_decay)
    else:
    ...
    for ep in range(max_epochs):
        ...
        for s_idx in range(0, N, batch_size):
            ...
            if use_sam:
                loss_val = _sam_step(model, opt, None, xb, yb, wb,
                                     rho=sam_rho, grad_clip=grad_clip)
                sched.step()
            else:
                preds = model(xb)
                sq = (preds - yb) ** 2
    ...
    if use_tta:
        print(f'  [V15.E TTA] running {tta_n}× noisy inference for val/test ...', flush=True)
        preds_val  = _predict_tta(model, val_data['X_grp'],  device, n_aug=tta_n)
        preds_test = _predict_tta(model, test_data['X_grp'], device, n_aug=tta_n)
    else:
        preds_val  = predict_grouped(model, val_data['X_grp'],  device, batch_size=1024)
    ...
    result = {
        ...
        'use_sam': bool(use_sam),
        'use_tta': bool(use_tta),
        'sam_rho': float(sam_rho), 'tta_n': int(tta_n),
        'n_train_groups': int(N_orig),
...
def main():
    ...
    ap.add_argument('--arch', type=str, default='v15_a_temporal')
    ...
    ap.add_argument('--out-root', type=str, default='runs_v15')
    ap.add_argument('--out-tag', type=str, default='v15_arch')
    args = ap.parse_args()
