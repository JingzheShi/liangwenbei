"""V7 training: V6_PairwiseInteract + Top50 interaction features (filtered).

Pipeline:
  1. Load grouped data (359d), prune to 259d via keep_idx_259d.npy.
  2. Compute Top50 interaction features (~116 per-sym channels) on the FULL 359d data.
  3. Select keep channels via selected_features.npy (boolean / int array).
  4. Per-channel z-norm using train mu/sd (inter_zstats.npz).
  5. Concat 259d + N_selected → train V6_PairwiseInteract with identical hyperparams.

Identical training protocol to train_v6.py:
  bs=1024, max_epochs=20, AdamW lr=1e-4 wd=1e-3, dropout=0.30,
  warmup=200, cosine eta_ratio=0.05, eval every 200, patience=1500.
"""
...
from models_v6 import build_model_v6, count_params
from interaction_features import compute_top50_features


def _build_inter_features(X_359, sel_idx, mu, sd, batch=200_000):
    """Compute interaction features in batches, select channels, z-normalize.

    X_359: (N, 5, 359) float32
    sel_idx: (K,) int64 channel indices to keep
    mu, sd: (n_channels,) — pre-computed z-stats over ALL channels
    Returns: (N, 5, K) float32
    """
    out_chunks = []
    n = X_359.shape[0]
    for s in range(0, n, batch):
        Xs = X_359[s:s + batch]
        Xi, _ = compute_top50_features(Xs)
        Xi = (Xi - mu) / sd
...
def train_v7(seed, device, keep_idx_npy, sel_idx_npy, zstats_npz,
             arch='v6_pairwise',
    ...
    print(f'\n=== [v7] arch={arch} seed={seed} keep_259={len(keep_idx)} sel_inter={len(sel_idx)} ===',
          flush=True)
    n_keep = len(keep_idx)
    n_sel  = len(sel_idx)
    ...
    # Compute interaction features on the FULL 359d (not pruned) versions
    print('  computing interaction features (train aug, val, test) ...', flush=True)
    t1 = time.time()
    ...
    run_name = f'{arch}_v7inter{n_sel}_s{seed}'
    if use_wandb:
        try:
            import wandb
    ...
    model = build_model_v6(arch, n_sym=5, n_feat=n_feat, **overrides).to(device)
    ...
    # don't save .pt to keep git clean
...
def main():
    ...
    ap.add_argument('--arch', type=str, default='v6_pairwise')
    ap.add_argument('--seed', type=int, default=0)
    ap.add_argument('--cuda', type=str, default='0')
    ap.add_argument('--keep-idx', type=str, default='keep_idx_259d.npy')
    ap.add_argument('--sel-idx',  type=str, default='selected_features.npy')
    ap.add_argument('--zstats',   type=str, default='inter_zstats.npz')
    ...
    ap.add_argument('--out-root', type=str, default='runs_v7')
    args = ap.parse_args()
