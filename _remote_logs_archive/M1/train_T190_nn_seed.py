"""T190: Extended 150 NN ensemble.
Seeds 1-50:  same HP as T188v2 (lr × dropout × batch cycling, hidden=(256,128,64)).
Seeds 51-150: extended HP grid (hidden variants, weight_decay variants, lr_min variants).
Phase 2: T170 SPO+ M7 protocol unchanged (lr=3e-5, λ_spo=30, 11 epochs fixed).
"""
...
# Phase 1 HP — original (seeds 1-50)
LR_LIST = [1e-4, 3e-4, 1e-3]
DROPOUT_LIST = [0.05, 0.10, 0.15, 0.20]
BATCH_LIST = [2048, 4096, 8192]
HIDDEN_DEFAULT = (256, 128, 64)

# Extended HP — seeds 51-150
LR_LIST_EXT = [1e-4, 3e-4, 1e-3, 5e-4, 2e-4]
HIDDEN_LIST_EXT = [
    (256, 128, 64),   # group 0 seeds 51-60
    (384, 192, 96),   # group 1 seeds 61-70
    (320, 160, 80),   # group 2 seeds 71-80
    (512, 256, 128),  # group 3 seeds 81-90
    (192, 96, 48),    # group 4 seeds 91-100
    (256, 128, 64),   # group 5 seeds 101-110
    (384, 192, 96),   # group 6 seeds 111-120
    (320, 160, 80),   # group 7 seeds 121-130
    (512, 256, 128),  # group 8 seeds 131-140
    (256, 128, 64),   # group 9 seeds 141-150
]
WD_LIST_EXT = [1e-5, 1e-5, 1e-5, 1e-4, 1e-4, 1e-4, 5e-4, 5e-4, 5e-4, 1e-5]
LR_MIN_LIST_EXT = [1e-6, 1e-5, 1e-6, 1e-5, 1e-6, 1e-5, 1e-6, 1e-5, 1e-6, 1e-5]

PHASE1_EPOCHS = 50
PHASE1_PATIENCE = 10
PHASE1_WD_DEFAULT = 1e-4
...
def get_hp(S: int):
    """Return (lr_p1, dropout, batch_p1, hidden, wd, lr_min) for seed S."""
    if S <= 50:
        ...
        wd = PHASE1_WD_DEFAULT
        lr_min = 1e-5
    else:
        idx = S - 51  # 0..99
        group = idx // 10   # 0..9
        within = idx % 10   # 0..9
        lr_p1 = LR_LIST_EXT[within % len(LR_LIST_EXT)]
        dropout = DROPOUT_LIST[within % len(DROPOUT_LIST)]
        batch_p1 = BATCH_LIST[within % len(BATCH_LIST)]
        hidden = HIDDEN_LIST_EXT[group]
        wd = WD_LIST_EXT[group]
        lr_min = LR_MIN_LIST_EXT[group]
    return lr_p1, dropout, batch_p1, hidden, wd, lr_min
...
class MLPRegr(nn.Module):
    def __init__(self, in_dim, hidden=HIDDEN_DEFAULT, dropout=0.10, use_layernorm=True):
        super().__init__()
        layers = []
        d = in_dim
        for h in hidden:
...
def extract_npz(ckpt, npz_path, hidden):
    sd = ckpt["state_dict"]
...
def main():
    ...
    S = args.seed
    H = args.horizon
    lr_p1, dropout, batch_p1, hidden, phase1_wd, lr_min = get_hp(S)

    print(f"\n{'='*60}", flush=True)
    print(f"T190 NN seed={S} h={H} lr_p1={lr_p1} dropout={dropout} "
          f"batch_p1={batch_p1} hidden={hidden} wd={phase1_wd} lr_min={lr_min}", flush=True)
    ...
    try:
        ...
        if not args.no_wandb:
            wandb.init(
                ...
                name=f"T190_NN_seed{S}",
                config=dict(seed=S, horizon=H, lr_p1=lr_p1, dropout=dropout,
                            batch_p1=batch_p1, hidden=list(hidden), wd=phase1_wd, lr_min=lr_min),
            )
    except Exception:
        pass
    ...
    # =========================================================
    # PHASE 1: Pretrain — train(0-79), val(80-95)
    # =========================================================
    if args.skip_phase1 and os.path.isfile(anchor_path):
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=PHASE1_EPOCHS, eta_min=lr_min
        )
        ...
        if best_epoch == 0:
            print(f"  WARNING: best_epoch=0 for seed {S}", flush=True)
        elif best_epoch < 5:
            print(f"  WARNING: best_epoch={best_epoch} < 5 for seed {S}", flush=True)
        else:
            print(f"  OK: best_epoch={best_epoch} — anchor converged", flush=True)
    ...
    target_scale = float(ckpt_p1["target_scale"])
    hidden_p2 = tuple(int(x) for x in ckpt_p1["hidden"])
    ...
    model = MLPRegr(feat_dim_p2, hidden=hidden_p2, dropout=dropout_p2,
    ...
    extract_npz(final_ckpt, npz_path, hidden_p2)
    ...
    try:
        ...
        if wandb.run is not None:
            wandb.log({"p2_time_sec": t_p2, "phase1_best_epoch": summary["phase1_best_epoch"]})
            wandb.finish()
    except Exception:
        pass
