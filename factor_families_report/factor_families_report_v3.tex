...
F1~\emph{LOB 派生}（105 维）、F2~\emph{多尺度 OFI}（50 维）、F3~\emph{订单流强度}（45 维）、
F4~\emph{微结构波动}（29 维）、F5~\emph{窗口统计}（67 维）、F6~\emph{不对称性}（74 维）。

\textbf{核心结论（feature importance 的唯一 bucket = F1--F6 unified）：}
\textbf{F1 占 LGB gain 40.4\%}、\textbf{F3 占 35.1\%}，两者合计 \textbf{75.5\%}，
是预测主力；F4 (7.5\%)、F6 (6.3\%)、F2 (5.5\%)、F5 (5.2\%) 单家族 gain 较低但承担多尺度冗余与 OOD 稳健化。
...
报告同时新增 §\ref{sec:nn-nan} 说明 NN 训练管线对 NaN 的处理：
...
\textbf{动机。}订单流不平衡（OFI, Cont 2014）是预测短期方向的最强信号家族。SchemeP 在 \emph{多窗口} ($W\in\{5,20,60\}$ for MLOFI；$W\in\{50,100\}$ for Kyle/toxicity) 与 \emph{多 EWMA 半衰期} ($\alpha\in\{0.05, 0.1, 0.3, 0.5\}$) 乘积空间内构造 50 维 OFI 表达。本家族 100\% 派生，无原始字段。
...
家族 45 维 = 原始 18 维 + 派生 27 维；LightGBM 给本家族 35.1\% 的 gain。Top 4 全局重要性都在 F3，分别是 \texttt{ma\_intst} 0.945、\texttt{mb\_intst} 0.860、\texttt{la\_intst} 0.650、\texttt{lb\_intst} 0.567，全部为原始字段。
...
也是\textbf{解决 OOD 漂移的核心}。所有 67 维全为派生（family-raw=0），把"绝对水平"通过 100-tick 窗口内的统计量
...
liq-asym 用 top-5 size 比值取对数；\texttt{*\_rate} 用滚动平均归一化每档价/量。家族 74 维 = 原始 40 维 (\texttt{*\_rate}) + 派生 34 维。
