% !TEX program = xelatex
% 因子工程报告 v2 —— 良文杯 2026 公榜/私榜 #1 方案 SchemeP
% v2 新增：F0 raw 154 各类型贡献分析 + NN NaN 处理
...
\title{\color{icmlblue}\textbf{因子工程报告（v2）}\\
\large 良文杯 2026 LOB 中间价方向预测——SchemeP 共 370 维因子拆解\\
\normalsize 新增 F0 原始 154 维各类型贡献分析 \& NN 训练管线 NaN 处理}
\author{史景喆（清华大学）\\
...
其中 \textbf{154 维原始字段（F0）} + \textbf{216 维派生因子（F1–F6 中除原始字段外的派生部分）}。

\textbf{核心结论（v2 新增）：} 把 370 维按"是否原始 vs 派生"切，LightGBM 的特征重要性显示
\textbf{F0 原始 154 维占总 gain 70.9\%}，派生 216 维仅占 29.1\%；
在 F0 内部最强单一类别是 \textbf{order flow intensity (\texttt{*\_intst})}，6 维贡献 raw-gain 31.3\%，
单因子 \texttt{ma\_intst} gain $= 0.95$ 居全特征第一。这一观察直接挑战了"派生因子是主力"的直觉——
派生层的真实角色更接近"OOD 稳健化器"，而非"信息核心"。

进一步按语义将 370 维归入六大家族（F1$\sim$F6）：
F1~\emph{LOB 派生}（105 维, 含 10 档原始价/量）、F2~\emph{多尺度 OFI}（50 维, 派生）、
F3~\emph{订单流强度}（45 维, 含 18 维原始 + 27 维派生）、F4~\emph{微结构波动}（29 维, 派生）、
F5~\emph{窗口统计}（67 维, 派生）、F6~\emph{不对称性}（74 维, 含 40 维原始 *\_rate）。
LightGBM 给 \textbf{F1 占 gain 40.4\%}、\textbf{F3 占 gain 35.1\%}，两者合计 75.5\%。
家族间最低共线性是 F5（块内 $|r|=0.07$、跨家族 $|r|<0.07$），这一独立性是 OOD 稳健性的几何基础。

报告同时新增一节 §\ref{sec:nn-nan} 说明 NN 训练管线对 NaN 的处理：
...
LGB 路径完全不做 NaN 处理（自带 missing 路由）。
...
% =========================================================================
\section{F0：原始 154 维各类型贡献分析}

\subsection{设计逻辑}

把 370 维按"是否派生"切，可以回答一个朴素而重要的问题：
\textbf{我们花了 216 维派生工程，到底带来了多少额外 gain？}
为此把 raw 154 维按字段语义拆成 17 个类型（OHLC、bid 价、ask 价、bid 量、ask 量、LOB 聚合、midprice、spread、bid\_diff、ask\_diff、LOB simple aggregates、价格 rate、量 rate、3 类 order-flow $\times$ 6 类事件），统计每个类型的 LGB gain 占比与单因子 gain。
计算基于 \texttt{big50\_lgb\_m7/seed=1} 模型（M7 重训第 1 个种子，370 维全量）。

\subsection{结果一：F0 vs 派生整体占比}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{f0_raw_vs_derived.pdf}
  \caption{\textbf{原始 154 维（F0）占 LGB 总 gain 70.9\%，派生 216 维仅占 29.1\%。}
  本图直接表明：派生因子对单一 LGB 模型的边际信息贡献并不大；其真正价值在于 OOD 稳健化（详见 §\ref{sec:discussion}）。}
  \label{fig:f0-vs-derived}
\end{figure}

\subsection{结果二：F0 内 17 个类型的 gain 分解}
...
LOB simple aggregates (bid\_mean/imbalance/cumspread/...) &  6 & 0.595 &  6.14 & 0.099 \\
...
\bottomrule
\end{tabular}
\caption{F0 raw 154 维按字段语义分组后的 LGB gain 占比与单因子均值。share 列以 raw-154 内部总 gain 为分母（即 9.687）。}
\label{tab:f0-cat}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{F0 raw 154 维：17 个类型 LGB gain 占比（以 raw-total = 100\% 标定），右侧标注 \emph{占比、维度数、平均单因子 gain}。\texttt{intst} 6 维一骑绝尘（31.3\%）；最末三类（midprice、price rate、volume/amount delta）几乎是死特征。}
\end{figure}

\subsection{结果三：order-flow 6$\times$3 子矩阵}

进一步把 18 维 order-flow 拆成 6 类事件（lb / la / mb / ma / cb / ca）$\times$ 3 类变体（intst / ind / acc）：

\begin{figure}[H]
  \centering
  \includegraphics[width=0.55\textwidth]{f0_orderflow_heatmap.pdf}
  \caption{F0 order-flow 18 维 gain heatmap。\texttt{ma\_intst}（市价买）gain $=0.945$ 全因子第一，\texttt{mb\_intst}（市价卖）0.860 第二。所有 6 类的 \texttt{\_acc} 变体几乎死特征（<0.05）；\texttt{cb/ca}（撤单）3 维 $\times$ 3 变体合计 gain $<0.01$，撤单"原始"信号 LGB 直接忽略，撤单 imbalance 已被 F3 派生的 \texttt{cancel\_imb\_W*} 接管。}
\end{figure}

\subsection{核心观察}

\begin{enumerate}[leftmargin=2em,itemsep=2pt,topsep=2pt]
  \item \textbf{Order flow intensity (\texttt{*\_intst}) 6 维是 F0 信息核心}：单因子最强（\texttt{ma\_intst} = 0.95、\texttt{mb\_intst} = 0.86），6 维合计占 raw-gain 31.3\%。这是 \emph{已脱敏的撮合事件计数}，比"由价量反推 OFI"直接得多。
  \item \textbf{LOB aggregate (4 维) 比 10 档原始价格更有用}：4 维占 raw-gain 15.0\%、平均单因子 gain 0.363，远高于 10 档 ask price（3.95\%、0.038）。原因：聚合量 \texttt{totalasize / totalbsize / avgbid / avgask} 已把 10 档信息压缩到 1 维有效信号，避免了"10 档分散重要性"。
  \item \textbf{midprice / spread / price\_rate 几乎死特征（每 feat $<0.01$）}：合计 40 维但只贡献 raw-gain 1.5\%。原因：midprice 严重共线（10 档 midprice 之间 $|r|>0.99$），spread 在标准化前是绝对值（受 sym 基差污染），price\_rate 已被 LOB derivative 主导。
  \item \textbf{volume / amount delta 几乎为 0（0.06\% raw-gain）}：原始单 tick 净流量信号已被 \texttt{*\_intst} 完整接管——后者是更精细的"按事件类型拆分"版本。这说明 F3 订单流强度的 6 类 \texttt{intst} 把 \emph{raw flow 信息} 全部"吃掉"了。
  \item \textbf{ind/acc 变体相比 intst 大幅退化}：6 类 \texttt{*\_ind} 合计 8.27\% raw-gain，\texttt{*\_acc} 仅 1.29\%。intst 的"强度"比"指示"或"加速度"信息更丰富。
\end{enumerate}
...
\textbf{注意：F0 与 F1–F6 不互斥}。F0 是按"原始 vs 派生"的 binary 切分，F1–F6 是按"语义功能"的 6-way 切分。每一维 raw feature 都同时属于 F0 和某个 F$i$（例：\texttt{ma\_intst} $\in$ F0 \emph{且} $\in$ F3；\texttt{bid1} $\in$ F0 \emph{且} $\in$ F1）。两种视角互补：F0 视角回答"派生工程值不值"，F1–F6 视角回答"信息维度上各家族的角色"。
...
\textbf{因子清单。}OHLC 4 + bid$k$/ask$k$ 20 + bsize$k$/asize$k$ 20 + avgbid/avgask/totalbsize/totalasize 4 + bid\_mean/ask\_mean/bsize\_mean/asize\_mean 4 + midprice$k$ 10 + spread$k$ 10 + cumspread/imbalance 2 + bid\_diff$k$/ask\_diff$k$ 20 + wmp\_lvl$k$ 10 + wmp\_balance\_12 1 = 105 维。
...
\textbf{动机。}订单流不平衡（OFI, Cont 2014）是预测短期方向的最强信号家族。SchemeP 在 \emph{多窗口} ($W\in\{5,20,60\}$ for MLOFI；$W\in\{50,100\}$ for Kyle/toxicity) 与 \emph{多 EWMA 半衰期} ($\alpha\in\{0.05, 0.1, 0.3, 0.5\}$) 乘积空间内构造 50 维 OFI 表达。

\textbf{因子清单。}mlofi\_W$W$\_lvl$k$ 30 + ewma\_ofi\_a$\alpha$\_lvl$k$ 12 + kyle\_inv\_W$W$ 2 + kyle\_lam\_W$W$ 2 + ofi\_tox\_lvl$k$\_W$W$ 4 = 50 维。
...
LightGBM 给本家族 35.1\% 的 gain，其中 6 维 \texttt{*\_intst} 单独贡献 22.2\% 总 gain（即 31.3\% raw-gain，见 F0 章节）。

\textbf{因子清单。}lb/la/mb/ma/cb/ca\_intst 6 + \_ind 6 + \_acc 6 + ewma\_a$\alpha$\_$c$\_intst 24 + cancel\_imb\_W$W$ 3 = 45 维。
...
\textbf{Top 4 全局重要性都在 F3。}LightGBM top features：\texttt{ma\_intst}（gain 0.95）、\texttt{mb\_intst}（0.86）、\texttt{la\_intst}（0.65）、\texttt{lb\_intst}（0.57）。模型最依赖的是 \emph{原始 4 维（market 与 limit 两类买卖强度）}，EWMA 滤波版本仅作为辅助。
...
\emph{活跃度突发} vol/amt burst。

\textbf{因子清单。}volume\_delta/amount\_delta 2 + rv\_w$W$ 4 + signed\_rv\_W$W$ 3 + rskew\_W$W$ 3 + signed\_bv\_W$W$ 3 + vol/amt\_burst\_W$W$ 4 + rv\_ratio 3 + jshare\_W$W$ 4 + roll\_eff\_spr\_ratio\_W$W$ 3 = 29 维。

\textbf{核心公式：}signed RV $= (\mathrm{RV}_+ - \mathrm{RV}_-)/(\mathrm{RV}_+ + \mathrm{RV}_-)\in[-1,1]$；
jump share $= \max(0, (RV_W - BV_W)/RV_W)$，$BV_W = \frac{\pi}{2}\sum |r_t||r_{t-1}|$（价格连续时 $BV\approx RV$，跳跃时 $RV \gg BV$）；
Roll 有效价差 $= 2\sqrt{\max(0, -\mathrm{cov}(\Delta P_t, \Delta P_{t-1}))}$。
...
\textbf{因子清单：}\texttt{dualz\_$c$} 37 + \texttt{qrank\_W100\_$c$} 20 + \texttt{mid\_ewma\_resid} 1 + \texttt{adapt\_mom\_W$W$} 3 + \texttt{spread\_reg\_W$W$} 3 + \texttt{trade\_pers\_W$W$} 3 = 67 维。
...
liq-asym 用 top-5 size 比值取对数；\texttt{*\_rate} 用滚动平均归一化每档价/量。

\textbf{因子清单：}\texttt{gofi\_W$W$\_lvl$k$} 30 + \texttt{liq\_asym\_top5\_W$W$} 4 + \texttt{bid\_rate$k$/ask\_rate$k$} 20 + \texttt{bsize\_rate$k$/asize\_rate$k$} 20\ \,= 74 维。
...
详见 §5.5 dualz 实现。\textbf{为什么对 OOD 关键：}
...
切分视角 & 类别 & gain 占比 (\%) & 备注 \\
\midrule
F0 vs derived (binary) & F0 raw 154        & \textbf{70.9} & 派生工程并非 gain 主力 \\
                       & derived 216        & 29.1          & 主要承担 OOD 稳健化 \\
\midrule
F1–F6 (6-way 语义)     & F1 LOB 派生 105    & 40.4          & 含原始 OHLC + 10 档 \\
                       & F2 多尺度 OFI 50   & 5.5           & 全派生 \\
                       & F3 订单流强度 45   & 35.1          & 含 18 维原始 *\_intst/ind/acc \\
                       & F4 微结构波动 29   & 7.5           & 主要派生 \\
                       & F5 窗口统计 67     & 5.2           & 全派生，OOD 主力 \\
                       & F6 不对称性 74     & 6.3           & 含 40 维原始 *\_rate \\
\bottomrule
\end{tabular}
\caption{两种视角下的 gain 占比。注意 F0 与 F$i$ 是不同的切分维度，不互斥。}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{LGB feature importance Top 40（按家族着色）。Top 4 全部为 F3 \texttt{ma/mb/la/lb\_intst}（原始 6 维 \texttt{*\_intst} 中的 4 个）；紧接着 F1 的 \texttt{asize1, totalasize, bsize1, avgbid/avgask, totalbsize, asize2}（全部为原始字段）。Top 20 几乎全部是 F0 原始字段。}
\end{figure}
...
而 F5 的 z-score 因子"个个不显眼，胜在彼此正交，组合起来形成稳健的决策边界"。单因子重要性低 $\ne$ 家族不重要。
...
\textbf{核心张力：}F0 章节表明 \emph{原始 154 维占 LGB gain 70.9\%}，那派生 216 维是不是"白做了"？答案是：单看单 LGB 模型 in-distribution 的 gain，派生贡献边际；但 OOD PnL 与多模型 ensemble 上，派生层（尤其 F5）撑起了稳健性。

\begin{enumerate}[leftmargin=2em,itemsep=2pt,topsep=2pt]
  \item \textbf{F5 窗口统计的独立性是 OOD 主力}：dualz / qrank 通过滚动归一化把跨股票/跨日基差消除，决策阈值在 OOD 上仍有效。F5 单因子 gain 平均 0.077（最低），但与所有家族近正交（$|r|<0.07$），它的 5.2\% gain 是"无冗余 5.2\%"。
  \item \textbf{F1 + F3 的高单因子信息量是 ID 主力}：F3 \texttt{*\_intst} 6 维占 raw-gain 31.3\%，F1 LOB aggregate 4 维占 raw-gain 15.0\%。在 in-distribution 阶段已经把"该容易的容易点"赚到极限。
...
\textbf{决策记录（v2 修订）：}
\begin{itemize}[leftmargin=2em,itemsep=2pt,topsep=2pt]
  \item \emph{F0 视角必要性}：v1 报告完全没有 F0 vs derived 视角，给读者错误印象"派生 216 维是主力"。v2 通过 §3 章节明确 F0 占 70.9\%，与"派生工程更多承担 OOD 稳健化"的真实角色保持一致。
...
\begin{itemize}[leftmargin=2em,itemsep=1pt,topsep=2pt]
  \item \textbf{F0 raw 154 维} 是单 LGB 模型 gain 主力（70.9\%）——其中 \texttt{*\_intst} 6 维 + LOB aggregate 4 维已占 raw-gain 46.3\%；
  \item \textbf{派生 216 维} 是 OOD 稳健化器，单看 gain 占比 29.1\% 边际，但 F5 z-score 的近正交性是跨股票/跨日泛化的几何基础；
